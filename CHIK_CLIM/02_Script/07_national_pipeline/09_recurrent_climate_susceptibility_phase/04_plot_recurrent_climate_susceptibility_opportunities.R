...
non_outbreak_points <- phase_points |>
  dplyr::filter(!major_wave_onset_next_8w)
...
contours <- bind_rows(lapply(contour_levels, function(level) {
  ...
  dplyr::filter(S_pre_median >= y_min, S_pre_median <= y_max)
...
trajectory_points <- opportunities |>
  dplyr::filter(state %in% PRIMARY_STATES, !is.na(R_climate_hat), !is.na(S_pre_median), !is.na(R_pred_median)) |>

...
source(file.path(shape_root(), "02_Script", "07_national_pipeline", "06_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
make_shape_input <- function(state, state_week, demography, foi_ensemble, settings) {
  annual_demography <- demography |>
    dplyr::filter(state == !!state, year %in% c(settings$years, max(settings$years) + 1L)) |>
    arrange(year)
  ...
  annual_cases <- state_week |>
    ...
    dplyr::filter(year %in% settings$years) |>
    group_by(year) |>
  ...
  annual <- tibble(year = settings$years, N_start = boundary_population[-length(boundary_population)],
    ...
    left_join(annual_demography |> dplyr::filter(year %in% settings$years) |>
...
fit_one_state_shape <- function(model, state_input, state, settings) {
  ...
  hmc_summary <- summary |> dplyr::filter(grepl("^(log_lambda_bar|log_sigma_year|z_year|log_kappa)", parameter))

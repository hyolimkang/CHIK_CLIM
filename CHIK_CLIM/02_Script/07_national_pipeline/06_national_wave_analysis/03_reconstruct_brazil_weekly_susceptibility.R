...
source(file.path(weekly_s_root(), "02_Script", "07_national_pipeline", "06_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
  ...
  for (y_index in seq_along(settings$years)) {
    ...
    if (!timing_unidentified) {
      ...
      population_week_start <- sapply(seq_len(nrow(year_weeks)), function(j) {
        ...
        interpolate_uf_population(demography |> dplyr::filter(state == !!state), year_weeks$week_start[j])
      })
...
extract_wave_onset_s <- function(census, reconstructions) {
  major <- census |> dplyr::filter(major_epidemic_primary)

...
source(file.path(early_re_root(), "02_Script", "07_national_pipeline", "06_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
build_window_batch <- function(major, weekly_cases, window, settings) {
  ...
  for (i in seq_len(nrow(major))) {
    ...
    wave <- major[i, ]; weekly <- dplyr::filter(weekly_cases, state == wave$state)
...
fit_window_batch <- function(model, batch, window, settings) {
  ...
  hmc_summary <- summary |> dplyr::filter(grepl("^(Re|phi)", parameter))
...
append_window_results <- function(output, batch, fitted, window) {
  bind_cols(output, bind_rows(lapply(seq_len(nrow(output)), function(i) {
    ...
    re_row <- dplyr::filter(fitted$summary, parameter == paste0("Re[", position, "]"))

...
training <- all_specs |> dplyr::filter(lag_spec == "primary")
...
lag_results <- bind_rows(lapply(unique(all_specs$lag_spec), function(spec_name) {
  d <- all_specs |> dplyr::filter(lag_spec == spec_name)
  fit <- fit_gam(d)
  sm <- summary(fit)
...
loo_curves <- bind_rows(lapply(ufs, function(excluded_uf) {
  d <- training |> dplyr::filter(UF != excluded_uf) |> mutate(UF = droplevels(UF))

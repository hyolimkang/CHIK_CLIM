...
build_dataset <- function(spec_name, spec) {
  bind_rows(lapply(seq_len(nrow(weekly_re)), function(i) {
    row <- weekly_re[i, ]
    climate_uf <- climate |> dplyr::filter(state == row$UF)
...
n_missing <- all_specs |> dplyr::filter(is.na(temperature) | is.na(precipitation)) |> nrow()
...
if (n_missing > 0) {
  ...
  print(as.data.frame(all_specs |> dplyr::filter(is.na(temperature) | is.na(precipitation)) |> select(UF, episode_id, window_weeks, week, lag_spec)))
}
all_specs_clean <- all_specs |> dplyr::filter(!is.na(temperature), !is.na(precipitation))

# Primary-spec training dataset (Section 5 GAM uses this)
training <- all_specs_clean |> dplyr::filter(lag_spec == "primary") |>

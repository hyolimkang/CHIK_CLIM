...
source(file.path(mc_root(), "02_Script", "07_national_pipeline", "06_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
run_provisional_gam_monte_carlo <- function() {
  ...
  re_draws <- readRDS(re_draws_path) |> dplyr::filter(window_weeks == 6, wave_id %in% episode$wave_id)
  s_draws <- readRDS(s_draws_path) |> dplyr::filter(wave_id %in% episode$wave_id)
  ...
  for (iter in seq_len(mc_settings$n_iterations)) {
    ...
    data_iter <- base |> inner_join(re_sample, by = "wave_id") |> inner_join(s_sample, by = "wave_id") |>
      dplyr::filter(Re_sample > 0) |> mutate(logRe = log(Re_sample), S_prop = S_sample)

...
source(file.path(episode_root(), "02_Script", "07_national_pipeline", "06_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
climate_window_summary <- function(climate_state, onset_week, window_offsets) {
  ...
  rows <- climate_state |> dplyr::filter(week_start %in% target_weeks)
...
run_build_episode_climate_dataset <- function() {
  ...
  if ("major_epidemic_primary" %in% names(master)) {
    population <- master |> dplyr::filter(major_epidemic_primary, re6_analysis_usable)
  } else {
    population <- master |> dplyr::filter(re6_analysis_usable)
  }
  ...
  for (i in seq_len(nrow(population))) {
    wave <- population[i, ]
    climate_state <- climate |> dplyr::filter(state == wave$state)
    ...
    s_row <- s_draws_summary |> dplyr::filter(state == wave$state, wave_id == wave$wave_id)
    if (!nrow(s_row)) {

...
match_primary_stability <- function(primary, sensitivity) {
  ...
  match_one <- function(row, def_name) {
    candidate <- dplyr::filter(sensitivity, definition == def_name, state == row$state)
...
summarise_definition <- function(census, definition_name, all_states) {
  ...
  states_two <- n_by_state |> dplyr::filter(n_episodes >= 2L) |> pull(state)
  re_by_state <- census |> dplyr::filter(re_prefit_eligible) |> count(state, name = "n_re")
  states_two_re <- re_by_state |> dplyr::filter(n_re >= 2L) |> pull(state)
  tibble(
...
ceara_quiet_audit <- function(state_week, primary_census, old_audit_path) {
  ce <- dplyr::filter(state_week, state == "CE", week_start >= as.Date("2016-01-01"), week_start <= as.Date("2017-12-31"))
...
write_visual_audit <- function(state_week, primary_census, ceara_result, output_path) {
  ...
  for (state_name in sort(unique(state_week$state))) {
    state_data <- dplyr::filter(state_week, state == state_name)
    state_episodes <- dplyr::filter(primary_census, state == state_name)
    print(episode_page(state_data, state_episodes, state_name))
  }
  ce_data <- dplyr::filter(state_week, state == "CE")
  ce_episodes <- dplyr::filter(primary_census, state == "CE")
  print(
...
build_brazil_chik_episode_census <- function() {
  ...
  primary <- dplyr::filter(all_census, definition == "primary") |>
    select(-definition) |>

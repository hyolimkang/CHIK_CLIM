...
panelD2 <- ggplot(year_summary, aes(x = year)) +
  geom_col(data = function(d) dplyr::filter(d, stable), aes(y = nnv_inf_med), fill = COL_INF, width = 0.6) +
  geom_errorbar(data = function(d) dplyr::filter(d, stable), aes(ymin = nnv_inf_lo, ymax = nnv_inf_hi), width = 0.15, linewidth = 0.4) +
  geom_text(data = function(d) dplyr::filter(d, !stable), aes(y = 0.06 * y_max_inf), label = "Insufficient accrued\nbenefit for stable NNV", size = 2.1, angle = 90, fontface = "italic", colour = "grey40", hjust = 0) +
  scale_x_discrete(limits = year_levels_chr) + scale_y_continuous(labels = comma_num) +

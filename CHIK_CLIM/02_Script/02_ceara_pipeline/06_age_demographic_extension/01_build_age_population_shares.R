...
age_pop <- readRDS(file.path(root, "01_Data/ibge_pop_uf_single_age_expanded_2015_2024.rds")) |>
  dplyr::filter(uf_code == CE_UF_CODE) |>
  arrange(year, age)
...
shares_2025 <- shares_by_year |> dplyr::filter(year == max(year)) |> mutate(year = 2025L)
...
print(as.data.frame(shares_full |> dplyr::filter(year %in% c(2015, 2020, 2024, 2025), age %in% c(0, 1, 12, 50, 100))), digits = 4)

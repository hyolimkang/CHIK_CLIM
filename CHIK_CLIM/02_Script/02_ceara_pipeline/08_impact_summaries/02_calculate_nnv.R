...
nnv_by_horizon <- bind_rows(lapply(names(horizon_idx), function(h) {
  idx <- horizon_idx[[h]]
  sub <- cum_by_draw_week |> dplyr::filter(t == idx) |>
...
nnv_year_end_by_draw <- bind_rows(lapply(names(year_end_idx), function(yr) {
  idx <- year_end_idx[[yr]]
  cum_by_draw_week |> dplyr::filter(t == idx) |>
...
# ================================================================
# REVISION (fig_nnv2, additional context panels): ANNUAL (non-cumulative)
# infections, no-vaccine vs vaccine, and their year-only difference
# I^NV_y - I^V_y. This is descriptive context for WHY the cumulative NNV
# bars move the way they do (Section 5 rebound) -- it is explicitly NOT an
# annual dose/effect NNV ratio (that remains disallowed per instruction;
# only counts and their difference are computed here, never divided by
# annual doses).
# ================================================================
annual_by_draw <- totals |> mutate(year = as.integer(format(week_start, "%Y"))) |> group_by(draw, year) |>
  summarise(infections_no_vaccine_annual = sum(X_A), infections_vaccine_annual = sum(X_B), .groups = "drop") |>
  mutate(infections_averted_annual = infections_no_vaccine_annual - infections_vaccine_annual) # I^NV_y - I^V_y; may be negative (rebound years)
saveRDS(annual_by_draw, file.path(DIR_RESULTS_NNV, "nnv_annual_by_draw.rds"))

table_nnv_annual <- annual_by_draw |> group_by(year) |>
  summarise(
    infections_no_vaccine_median = median(infections_no_vaccine_annual), infections_no_vaccine_lo95 = quantile(infections_no_vaccine_annual, .025), infections_no_vaccine_hi95 = quantile(infections_no_vaccine_annual, .975),
    infections_vaccine_median = median(infections_vaccine_annual), infections_vaccine_lo95 = quantile(infections_vaccine_annual, .025), infections_vaccine_hi95 = quantile(infections_vaccine_annual, .975),
    infections_averted_median = median(infections_averted_annual), infections_averted_lo95 = quantile(infections_averted_annual, .025), infections_averted_hi95 = quantile(infections_averted_annual, .975),
    pr_infections_averted_gt0 = mean(infections_averted_annual > 0),
    .groups = "drop"
  )
write_csv(table_nnv_annual, file.path(DIR_TABLES_NNV, "table_nnv_annual.csv"))
message("\n=== TABLE: annual (non-cumulative) infections and infections averted ===")
print(as.data.frame(table_nnv_annual), digits = 4)
if (any(table_nnv_annual$infections_averted_hi95 < 0 | table_nnv_annual$infections_averted_lo95 > 0)) {
  neg_years <- table_nnv_annual$year[table_nnv_annual$infections_averted_median < 0]
  if (length(neg_years)) message("[note] Years with NEGATIVE median annual infections averted (rebound -- vaccine arm has MORE infections that year): ", paste(neg_years, collapse = ", "))
}
...
baseline_S_total <- totals |> dplyr::filter(t == 1) |> transmute(draw, S_total_N_baseline = S_A / N_A)
...
spearman_cors <- nnv_susceptibility |> dplyr::filter(is.finite(NNV_infection)) |>

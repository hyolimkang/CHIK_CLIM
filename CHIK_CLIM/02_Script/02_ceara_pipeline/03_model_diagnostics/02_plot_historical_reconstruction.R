# Established-convention 6-panel trajectory figure for the CE
# climate-forced canary fit -- SAME layout/theme/panels as
# 28_v4_9_hierarchical_seasonality/scripts/03_plot_v4_9_six_panel.R
# (fixed-q version: panel E shows the fixed q line, panel F carries the
# Juazeiro serology anchor). Only cosmetic differences: title/subtitle
# note the climate-forced structure; R0(t) panel subtitle notes that
# R0(t) already includes the fitted climate_multiplier(t) internally
# (no separate panel needed -- R0_t is the post-climate-forcing value).
...
plot_ce_climate_canary_six_panel <- function() {
  ...
  qval <- bundle$config$q
  seed_xmin <- min(bundle$seed_dates); seed_xmax <- max(bundle$seed_dates) + 7

  pars <- c("X", "S_prop", "immune_prop", "R0_t", "R_eff_t", "expected_reported_cases", "C_pred", "beta_T", "beta_P")
  ...
  p_reproduction <- ggplot(reproduction, aes(week_start, colour = number, fill = number)) +
    ...
    x_scale +
    labs(title = "Time-varying reproduction numbers",
         subtitle = sprintf("R0(t) INCLUDES fitted climate forcing: beta_T=%.3f, beta_P=%.3f (posterior medians)", median(draws$beta_T), median(draws$beta_P)),
  ...
  p_ascertainment <- ggplot(ascertainment_df, aes(week_start, q)) +
    ...
    x_scale +
    labs(title = "Case ascertainment", subtitle = sprintf("q FIXED at %.2f (unchanged from frozen v4.9 -- not re-estimated here)", qval),
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(
      title = "Chikungunya transmission in Cear\u00e1 -- climate-forced v4.9 canary (2015-2025)",
      subtitle = sprintf("v4.9 + climate forcing on log_R0(t), q = %.2f fixed (%s) | 2022 seed conditioned",
  ...
  table_path <- file.path(table_dir, paste0(output_base, "_summary.csv"))
...
if (sys.nframe() == 0L) plot_ce_climate_canary_six_panel()

fig <- plot_ce_climate_canary_six_panel()
fig

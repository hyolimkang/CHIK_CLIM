...
transformed parameters {
  ...
  for (t in 2:N) {
    ...
    new_inf_frac[t] = new_inf_count[t] / pop[t];

    p_rec = 1.0 - exp(-gamma_fixed);
    recov_count = I_prev_count * p_rec;

    S_frac[t] = (S_prev_count - new_inf_count[t] + births_weekly[t]) / pop[t];
    I_frac[t] = (I_prev_count + new_inf_count[t] - recov_count) / pop[t];
    R_frac[t] = (R_prev_count + recov_count) / pop[t];

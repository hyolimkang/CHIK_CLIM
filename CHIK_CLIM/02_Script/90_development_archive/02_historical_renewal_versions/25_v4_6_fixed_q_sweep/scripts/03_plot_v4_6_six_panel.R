# Six-panel trajectory figure for a single v4.6 fixed-q fit, in the same
# format/layout as 02_v2/03_plot_renewal_v2_fit.R (the original "smoke
# hazard" figure). Differences from v2, reflecting v4.6's actual model:
#   - Panel E ("case ascertainment") shows q as a horizontal constant
#     (q is FIXED DATA in v4.6, not time-varying rho_sym_t/rho_total_t as
#     in v2's hierarchical symptomatic/reporting decomposition).
#   - Panel F compares the model's Ceara-level immune_prop directly against
#     observed Juazeiro serology (v4.6 has no separate hierarchical
#     site-level attack-rate parameter, unlike v2).
#
# Usage: QVAL=0.05 Rscript 03_plot_v4_6_six_panel.R
...
base_dir <- file.path(root, "02_Script/90_development_archive/02_historical_renewal_versions/25_v4_6_fixed_q_sweep")

plot_v4_6_six_panel <- function(qval) {
  tag <- paste0("q", sprintf("%.2f", qval))
  bundle <- readRDS(file.path(base_dir, "outputs", tag, paste0("renewal_ceara_v4_6_fit_", tag, ".rds")))
  fit <- bundle$fit
  ...
  p_immunity <- ggplot(immunity, aes(week_start, colour = state, fill = state)) +
    ...
    x_scale +
    labs(title = "Population susceptibility and immunity", subtitle = "Closed population; lifelong immunity",
  ...
  p_reproduction <- ggplot(reproduction, aes(week_start, colour = number, fill = number)) +
    ...
    x_scale +
    labs(title = "Time-varying reproduction numbers", subtitle = expression(R[eff](t) == R[0](t) %*% S(t - 1) / N),
  ...
  p_ascertainment <- ggplot(ascertainment_df, aes(week_start, q)) +
    ...
    x_scale +
    labs(title = "Case ascertainment", subtitle = sprintf("q FIXED at %.2f (not estimated -- v4.6 fixed-q sweep)", qval),
  ...
  p_attack <- ggplot(attack, aes(week_start)) +
    ...
    geom_errorbar(data = sero_observed_df, aes(week_start, ymin = lower, ymax = upper, colour = "Observed Juazeiro seroprevalence"), width = 18, linewidth = 0.55) +
    geom_point(data = sero_observed_df, aes(week_start, estimate, colour = "Observed Juazeiro seroprevalence"), shape = 18, size = 2.3) +
    geom_errorbar(data = model_at_sero, aes(week_start, ymin = q025, ymax = q975, colour = "Model Ceara immune fraction (at survey)"), width = 18, linewidth = 0.55, position = position_nudge(x = 25)) +
    geom_point(data = model_at_sero, aes(week_start, median, colour = "Model Ceara immune fraction (at survey)"), shape = 16, size = 1.8, position = position_nudge(x = 25)) +
    ...
    x_scale +
    labs(title = "Cumulative infection and serological anchor", subtitle = "Yellow field: June-December 2018 survey; no site-level hierarchy in v4.6",
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(
      ...
      subtitle = sprintf("v4.6 fixed-q sweep, q = %.2f (%s) | weekly observations, 2015-2019", qval, if (bundle$hmc$hmc_pass) "HMC PASS" else "HMC FAIL/marginal"),
      tag_levels = "A",
      theme = theme(
  ...
  table_dir <- file.path(root, "03_Output/tables/renewal_v4_6_fixed_q_sweep")
  dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(table_dir, recursive = TRUE, showWarnings = FALSE)

  output_base <- paste0("renewal_v4_6_trajectories_ceara_", tag)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("renewal_v4_6_trajectory_summary_", tag, ".csv"))

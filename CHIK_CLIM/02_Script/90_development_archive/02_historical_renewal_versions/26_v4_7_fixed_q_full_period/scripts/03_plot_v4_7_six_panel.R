# Six-panel trajectory figure for a single v4.7 fixed-q FULL-PERIOD fit,
# same format as 25_v4_6.../03_plot_v4_6_six_panel.R. Usage: QVAL=0.05 Rscript 03_plot_v4_7_six_panel.R
...
base_dir <- file.path(root, "02_Script/90_development_archive/02_historical_renewal_versions/26_v4_7_fixed_q_full_period")

plot_v4_7_six_panel <- function(qval) {
  tag <- paste0("q", sprintf("%.2f", qval))
  bundle <- readRDS(file.path(base_dir, "outputs", tag, paste0("renewal_ceara_v4_7_fit_", tag, ".rds")))
  fit <- bundle$fit
  ...
  p_cases <- ggplot() +
    ...
    labs(title = "Reported cases and posterior prediction",
         subtitle = "Shading: 95% predictive interval; dark band: 50% CrI. Note the 2022 recurrence.",
  ...
  p_immunity <- ggplot(immunity, aes(week_start, colour = state, fill = state)) +
    ...
    x_scale +
    labs(title = "Population susceptibility and immunity", subtitle = "Closed population; LIFELONG immunity assumed",
  ...
  p_ascertainment <- ggplot(ascertainment_df, aes(week_start, q)) +
    ...
    x_scale +
    labs(title = "Case ascertainment", subtitle = sprintf("q FIXED at %.2f (not estimated -- v4.7 fixed-q sweep, full period)", qval),
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(
      ...
      subtitle = sprintf("v4.7 fixed-q sweep, q = %.2f (%s) | weekly observations, full period", qval, if (bundle$hmc$hmc_pass) "HMC PASS" else "HMC FAIL/marginal"),
      tag_levels = "A",
      theme = theme(
  ...
  table_dir <- file.path(root, "03_Output/tables/renewal_v4_7_fixed_q_full_period")
  dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(table_dir, recursive = TRUE, showWarnings = FALSE)

  output_base <- paste0("renewal_v4_7_trajectories_ceara_", tag)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("renewal_v4_7_trajectory_summary_", tag, ".csv"))

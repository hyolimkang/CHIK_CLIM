...
summarise_trajectory <- function(draw_matrix, variable) {
  ...
  tibble::tibble(
    variable = variable, t = seq_len(ncol(draw_matrix)),
...
run_v3_2_diagnostics <- function() {
  ...
  gate_summary <- data.frame(
    metric = c("divergences", "treedepth_hits", "max_rhat", "rhat_gt_1.01", "min_bulk_ess", "min_tail_ess", "elapsed_seconds", "gate_pass"),
  ...
  grDevices::pdf(trace_path, width = 11, height = 8.5)
  par(mfrow = c(4, 2))
  for (p in dimnames(trace_draws)[[3]]) {
    matplot(trace_draws[, , p],
      type = "l", lty = 1,
  ...
  text(.05, .83, if (gate) "PASS: epidemiological figures may be inspected" else "FAIL: do not interpret posterior trajectories",
    adj = c(0, 1), cex = 1.1,
  ...
  p_cases <- ggplot() +
    ...
    x_scale +
    labs(title = "Reported cases and posterior prediction", subtitle = "Shading: 95% predictive interval; dark band: 50% CrI", x = NULL, y = "Weekly reported cases") +
    theme_publication()

  latent <- get_summary("latent_infections")
  p_latent <- interval_layers(ggplot(latent, aes(week_start)), latent, COL_VERMILLION) + x_scale +
    scale_y_continuous(labels = scales::label_number(big.mark = ","), expand = expansion(mult = c(0, .06))) +
    labs(title = "Latent infection incidence", subtitle = "Cear<U+00E1> total: median, 50% and 95% credible intervals", x = NULL, y = "True infections per week") + theme_publication()
  ...
  p_immunity <- ggplot(immunity, aes(week_start, colour = state, fill = state)) +
    geom_ribbon(aes(ymin = q025, ymax = q975), alpha = .10, colour = NA) +
    geom_line(aes(y = median), linewidth = .7) +
    ...
    scale_y_continuous(limits = c(0, 1), labels = scales::label_percent(accuracy = 1), breaks = seq(0, 1, .25), expand = expansion(mult = c(0, .01))) +
    x_scale +
    labs(title = "Population susceptibility and immunity", subtitle = "Population-weighted Cear<U+00E1> total", x = NULL, y = "Population proportion") +
    theme_publication()

  reproduction <- bind_rows(get_summary("shared_R0") |> filter(t > stan_data$seed_weeks) |> mutate(number = "Shared R0(t)"), get_summary("Reff_ceara") |> filter(t > stan_data$seed_weeks) |> mutate(number = "Cear<U+00E1> Reff(t)"))
  p_reproduction <- ggplot(reproduction, aes(week_start, colour = number, fill = number)) +
    geom_hline(yintercept = 1, colour = COL_MUTED, linewidth = .35, linetype = "22") +
    geom_ribbon(aes(ymin = q025, ymax = q975), alpha = .09, colour = NA) +
    geom_line(aes(y = median), linewidth = .65) +
    scale_colour_manual(values = c("Shared R0(t)" = COL_NAVY, "Cear<U+00E1> Reff(t)" = COL_VERMILLION)) +
    scale_fill_manual(values = c("Shared R0(t)" = COL_NAVY, "Cear<U+00E1> Reff(t)" = COL_VERMILLION)) +
    scale_y_continuous(expand = expansion(mult = c(.03, .06))) +
    x_scale +
    labs(title = "Transmission potential", subtitle = "Shared R0 trajectory and population-weighted Cear<U+00E1> Reff", x = NULL, y = "Reproduction number") +
    theme_publication()

  q_ci <- stats::quantile(draws$q, c(.025, .5, .975))
  p_reporting <- ggplot(tibble(q = draws$q), aes(y = q)) +
    geom_boxplot(fill = COL_ORANGE, alpha = .65, width = .25, outlier.alpha = .12) +
    scale_y_continuous(labels = scales::label_percent(accuracy = .1), limits = c(0, NA)) +
    labs(title = "Pooled infection-to-report probability", subtitle = sprintf("q = %.3f [%.3f, %.3f]", q_ci[2], q_ci[1], q_ci[3]), x = NULL, y = "Probability") +
    theme_publication()

  serology <- prepared$serology
  sero_obs <- tibble(
    site = serology$site, observed = stan_data$sero_positive / stan_data$sero_n,
  ...
  p_serology <- ggplot(sero_plot, aes(site, median)) +
    geom_errorbar(aes(ymin = q025, ymax = q975, colour = "Model site seroprevalence"), width = .15, linewidth = .6) +
    geom_point(aes(colour = "Model site seroprevalence"), size = 2) +
    geom_errorbar(aes(ymin = lower, ymax = upper, colour = "Observed seroprevalence"), width = .15, linewidth = .6, position = position_nudge(x = .12)) +
    geom_point(aes(y = observed, colour = "Observed seroprevalence"), size = 2, shape = 18, position = position_nudge(x = .12)) +
    scale_colour_manual(values = c("Model site seroprevalence" = COL_PURPLE, "Observed seroprevalence" = COL_ORANGE)) +
    scale_y_continuous(labels = scales::label_percent(accuracy = 1), limits = c(0, 1)) +
    labs(title = "Direct local serology validation", subtitle = "Model: regional survey-window mean; bars: 95% intervals", x = NULL, y = "Cumulative proportion infected") +
    theme_publication()

  figure_subtitle <- if (gate) {
    "v3.2 coarse-regional renewal pilot: statewide population-weighted trajectory"
  } else {
    "EXPLORATORY ONLY <U+2014> UNCONVERGED CHAINS; DO NOT INTERPRET AS POSTERIOR INFERENCE"
  }
  ceara_page <- (p_cases | p_latent) / (p_immunity | p_reproduction) / (p_reporting | p_serology) +
    plot_annotation(title = "Chikungunya transmission and susceptibility in Cear<U+00E1>, Brazil", subtitle = figure_subtitle, tag_levels = "A", theme = theme(text = element_text(family = "Arial", colour = COL_INK), plot.title = element_text(size = 12.5, face = "bold"), plot.subtitle = element_text(size = 9.5, colour = if (gate) COL_MUTED else COL_VERMILLION, face = if (gate) "plain" else "bold"), plot.tag = element_text(size = 11, face = "bold")))
  ...
  regional_page <- ggplot(regional_s, aes(week_start)) +
    ...
    geom_ribbon(aes(ymin = q25, ymax = q75), fill = COL_GREEN, alpha = .24, colour = NA) +
    geom_line(aes(y = median), colour = COL_GREEN, linewidth = .6) +
    ...
    theme_publication() +
    theme(strip.background = element_blank(), strip.text = element_text(face = "bold", colour = COL_INK), legend.position = "none")

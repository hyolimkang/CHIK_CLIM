...
source(here::here("02_Script/90_development_archive/02_historical_renewal_versions/05_v3_0_spatial/prepare_renewal_v3_0_spatial_data.R"))

v3_2_units <- c(
  "Fortaleza", "Fortaleza_remainder", "Norte_Sobral",
...
derive_q_prior <- function(n_draws = 1000000L, seed = 32052018L) {
  ...
  mu <- mean(q_draws)
  va <- stats::var(q_draws)
  concentration <- mu * (1 - mu) / va - 1
  list(
    a = mu * concentration, b = (1 - mu) * concentration,
...
prepare_renewal_v3_2_coarse_regional_data <- function() {
  ...
  mapping <- utils::read.csv(map_path,
    stringsAsFactors = FALSE, colClasses = "character",
    check.names = FALSE
  )
  needed <- c(
    "muni6", "municipality_name", "health_region_22",
  ...
  if (any(C < 0) || any(abs(C - round(C)) > 1e-8) ||
    any(births < 0) || any(deaths < 0)) {
    stop("Invalid aggregated cases or demographic flows")
  }
...
if (sys.nframe() == 0) {
  ...
  saveRDS(x, out)
  message(sprintf(
    "[save] %s | %d units, %d weeks, S+U error %.3g", out,

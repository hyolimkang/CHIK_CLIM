# Post-hoc strict HMC gate check for one completed q-node fit.
#
...
# every chain. Also reports min tail ESS, treedepth quantiles, and
# chain-specific diagnostics, exactly as for v4.0. Also applies the
# >3x-benchmark "COMPUTATIONALLY_EXPENSIVE" runtime flag (Section 9).
#
# Usage: Q_NODE_ID=q50 Rscript 04_check_hmc_gate.R
...
run_check_hmc_gate <- function(node_id = Sys.getenv("Q_NODE_ID", "q50")) {
  root <- gate_root()
  out_dir <- file.path(root, "02_Script", "90_development_archive", "02_historical_renewal_versions", "19_v4_1_modular_q", "outputs", node_id)
  ...
  core_pars <- c("alpha_R", "z_year", "beta_sin", "beta_cos", "phi_obs")
  ...
  write_csv(hmc, file.path(out_dir, paste0("hmc_diagnostics_", node_id, ".csv")))
  write_csv(arrange(parameter_summary, desc(rhat)), file.path(out_dir, paste0("parameter_diagnostics_", node_id, ".csv")))
  write_csv(chain_diag, file.path(out_dir, paste0("chain_specific_diagnostics_", node_id, ".csv")))
  write_csv(treedepth_quantiles, file.path(out_dir, paste0("treedepth_quantiles_", node_id, ".csv")))

  runtime_path <- file.path(out_dir, paste0("runtime_", node_id, ".csv"))
  runtime_flag <- if (file.exists(runtime_path)) read_csv(runtime_path, show_col_types = FALSE)$flag[1] else NA_character_

  message(sprintf("[%s] STRICT HMC GATE: %s", node_id, if (hmc_pass) "PASS" else "FAIL"))
  message(sprintf("  divergences=%d | max_treedepth_hits=%d | max_Rhat=%.5f | min_bulk_ESS=%.1f | min_tail_ESS=%.1f | BFMI range=[%.3f, %.3f]",
                   ...
                   min(bfmi), max(bfmi)))
  if (!is.na(runtime_flag)) message(sprintf("  runtime flag: %s", runtime_flag))
  if (!hmc_pass) {
    message("  *** GATE FAILED. Per Section 19: do NOT delete this node's output, do NOT loosen the gate. Report before proceeding. ***")
  }
  invisible(list(hmc = hmc, hmc_pass = hmc_pass, chain_diag = chain_diag, treedepth_quantiles = treedepth_quantiles, runtime_flag = runtime_flag))
}

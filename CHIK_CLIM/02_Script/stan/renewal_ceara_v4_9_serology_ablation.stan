...
// Byte-identical to renewal_ceara_v4_9_hierarchical_seasonality.stan with
// EXACTLY ONE change: the Juazeiro serology likelihood contribution
// (`sero_pos ~ beta_binomial(...)`) is removed from the model block.
// Everything else -- data, transformed parameters (including
// p_state_sero_at_anchor/alpha_sero/beta_sero, still computed for
// generated-quantities comparison), priors, the 2022 seed mechanism, and
// generated quantities -- is unchanged, so log_lik_serology/sero_pred can
// still be inspected post hoc even though they no longer influence the
// posterior. Purpose: determine whether the Juazeiro serology anchor is
// constraining/suppressing the 2017 epidemic burden (see
// V5_0_DYNAMIC_R_ASSESSMENT.md Section 2-3).
...
model {
  ...
  // NOTE: sero_pos ~ beta_binomial(...) is DELIBERATELY OMITTED (ablation).
}

generated quantities {
  array[N] int C_pred;
  vector[N] log_lik_cases;
  real log_lik_serology; // computed for comparison only; not part of target
  int sero_pred;
  for (t in 1:N) {

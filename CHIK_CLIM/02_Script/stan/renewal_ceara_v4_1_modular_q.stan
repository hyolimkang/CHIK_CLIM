// Ceará v4.1 (modular q): the HMC-passing v4.0/td14 renewal backbone,
// UNCHANGED except that q_fixed is renamed q_external for clarity.
//
// This file is verified byte-for-byte identical to
// renewal_ceara_v4_0_minimal_no_vaccine.stan (aside from that rename) by
// 19_v4_1_modular_q/scripts/00_audit_v4_0_reference.R -- run that script
// after editing this file to re-confirm equivalence before any fit.
//
// q_external is DATA, not a parameter, in every conditional fit. Reporting-
// fraction uncertainty is propagated MODULARLY: q_external is varied across
// five separate fits (one per external-prior quadrature node; see
// 01_build_q_nodes.R), not estimated jointly with transmission parameters
// within a single fit. This is a deliberate choice to avoid NUTS having to
// explore the q <-> R0 <-> X <-> S ridge, which made an earlier joint-q
// attempt (18_v4_1_minimal_no_vaccine/) operationally impractical (>5x
// slower than this proven backbone, aborted before diagnostics were
// obtained). See 19_v4_1_modular_q/DEVELOPMENT_LOG.md for the full
// rationale. This is intentionally NOT a fully joint Bayesian posterior in
// q: see 05_combine_modular_posterior.R for how the resulting ensemble
// should and should not be described.
//
// It does not estimate reporting, introductions, climate effects, serology
// offsets, or a weekly latent R process. sigma_year and the Y-1 orthonormal
// annual-effect basis from the aborted joint-q attempt are NOT part of this
// model -- year_effect_prior_sd remains an externally fixed scale, exactly
// as in the HMC-passing v4.0/td14 run.
...
data {
  ...
  real<lower=0, upper=1> q_external;
...
transformed parameters {
  ...
  for (t in 1:N) {
    ...
    expected_reported_cases[t] = q_external * X[t] + 1e-9;

// v4.6: q is FIXED DATA (swept across a pre-registered grid), not an
// estimated parameter -- the direct reversal of v4.1-v4.5's attempts to
// jointly estimate q, following the mode-validity audit's conclusion that
// case counts + one serosurvey point cannot identify q regardless of how
// the observation likelihood is structured (weekly NB, 4-week block NB, or
// yearly-burden + within-year Dirichlet-multinomial all showed persistent
// or newly-collapsed-but-unresolved bimodality). This restores the simple,
// cleanly HMC-passing weekly structure of v4.0/td14 (extended to include
// the Juazeiro 2018 serology likelihood, as in v4.2 Stage B4), on the same
// 2015-2019 window as v4.3-v4.5, so that q's effect on fit quality can be
// read off directly by comparing independent fits across the q grid
// instead of by joint estimation.
//
// The latent weekly renewal process (X[t], S[t], U[t], R0_t, GI, seasonality,
// annual effects, imports) and the Juazeiro serology treatment (beta-binomial,
// fixed kappa_sero) are unchanged from v4.3/v4.4/v4.5.
...
data {
  ...
  real<lower=0, upper=1> q; // FIXED, swept across a grid -- not estimated
}

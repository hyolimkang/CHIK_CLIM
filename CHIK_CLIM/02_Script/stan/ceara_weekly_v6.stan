...
model {
  ...
  // Slightly wider than v5.
  amp_year ~ lognormal(log(1.0), 0.5);

  // Year-specific effective importation / outbreak pressure.
  // Wider than v5 to reduce bound-constrained underfit.
  ...
  // Observation overdispersion.
  // Keep lower=1 for now; relax only if it still hits the lower bound after v6.

// Ceará v4.2: the HMC-passing v4.0/td14 renewal backbone (verified
// byte-for-byte identical, aside from q, by
// 21_v4_2_q_juazeiro_serology/scripts/00_verify_ablation_identity.R) plus:
//
//   (a) q_fixed (data) -> q (parameter), Beta(16.2, 108.7) prior -- exactly
//       as in the failed q-only ablation (20_v4_1_q_only/), and
//   (b) ONE serology likelihood term: the Juazeiro do Norte 2018
//       population-based seroprevalence survey (Barreto et al. 2020, BMC
//       Infectious Diseases; verified against the primary source: n=404,
//       103 CHIKV IgM/IgG positive, June-December 2018, random spatial
//       point sampling proportional to neighbourhood population).
//
// This is a minimal identifiability experiment: does ONE independent
// cumulative-infection observation stabilise q's estimation without adding
// any other flexibility? Everything else (annual effects via fixed
// year_effect_prior_sd, seasonality, generation interval, imports_per_week,
// S/U bookkeeping, NB case likelihood) is UNCHANGED from v4.0/td14.
//
// Juazeiro do Norte is a MUNICIPALITY, not a random sample of the whole
// state -- so the serology likelihood is deliberately a beta-binomial with
// a FIXED (not estimated) overdispersion kappa_sero=50, not a naive
// binomial(404, p_state_sero), to avoid treating within-municipality
// sampling precision as if it were state-level representativeness
// precision. See 06_kappa_calibration.R for the prior-predictive check of
// this choice.

data {
  int<lower=2> N;
  int<lower=1> G;
  array[N] int<lower=0> C;
  simplex[G] w;
  vector<lower=0>[N] N_start;
  vector<lower=0>[N] N_end;
  vector<lower=0>[N] births;
  vector<lower=0>[N] deaths;
  int<lower=1> Y;
  array[N] int<lower=1, upper=Y> year_id;
  vector[N] seasonal_sin;
  vector[N] seasonal_cos;
  real<lower=0> imports_per_week;
  real<lower=0> year_effect_prior_sd;
  // Serology anchor (Juazeiro do Norte, 2018 -- Section 4).
  int<lower=1, upper=N> t_sero;      // week index nearest the survey midpoint
  int<lower=0> sero_pos;             // 103
  int<lower=1> sero_n;               // 404
  real<lower=0> kappa_sero;          // FIXED representativeness-weakening dispersion (data, not estimated); primary run = 50
}

transformed data {
  vector[N] reconciliation;
  for (t in 1:N) {
    reconciliation[t] = N_end[t] - N_start[t] - births[t] + deaths[t];
  }
}

parameters {
  real alpha_R;
  vector[Y] z_year;
  real beta_sin;
  real beta_cos;
  real<lower=0> phi_obs;
  real<lower=0, upper=1> q;
}

transformed parameters {
  vector[Y] year_effect;
  vector[N] log_R0;
  vector[N] R0_t;
  vector[N] R_eff_t;
  vector[N] force_of_infection;
  vector[N] X;
  vector[N] S;
  vector[N] U;
  vector[N] S_prop;
  vector[N] immune_prop;
  vector[N] expected_reported_cases;
  real p_state_sero_at_anchor;
  real p_sero_safe;
  real alpha_sero;
  real beta_sero;

  year_effect = year_effect_prior_sd * (z_year - mean(z_year));
  S[1] = N_start[1];
  U[1] = 0;

  for (t in 1:N) {
    real total = S[t] + U[t];
    real infectiousness = 0;

    if (S[t] < 0 || U[t] < 0 || fabs(total - N_start[t]) > 1e-6 * fmax(1, N_start[t])) {
      reject("Invalid demographic state at week ", t);
    }
    for (g in 1:G) {
      if (t > g) infectiousness += w[g] * X[t - g];
    }

    log_R0[t] = alpha_R + year_effect[year_id[t]] +
      beta_sin * seasonal_sin[t] + beta_cos * seasonal_cos[t];
    R0_t[t] = exp(log_R0[t]);
    force_of_infection[t] = R0_t[t] * infectiousness / total + imports_per_week / total;
    X[t] = S[t] * (-expm1(-force_of_infection[t]));
    R_eff_t[t] = R0_t[t] * S[t] / total;
    S_prop[t] = S[t] / total;
    immune_prop[t] = U[t] / total;
    expected_reported_cases[t] = q * X[t] + 1e-9;

    if (t < N) {
      real susceptible_after_infection = S[t] - X[t];
      real immune_after_infection = U[t] + X[t];
      real susceptible_fraction = susceptible_after_infection /
        (susceptible_after_infection + immune_after_infection);
      S[t + 1] = susceptible_after_infection + births[t] -
        deaths[t] * susceptible_fraction + reconciliation[t] * susceptible_fraction;
      U[t + 1] = immune_after_infection - deaths[t] * (1 - susceptible_fraction) +
        reconciliation[t] * (1 - susceptible_fraction);
    }
  }

  // Model-implied Ceará state seroprevalence at the survey anchor week
  // (Section 6): U/(S+U) = 1 - S/N under lifelong infection-derived
  // immunity. Numerically safe clamp (Section 9) before beta-binomial shape
  // construction.
  p_state_sero_at_anchor = immune_prop[t_sero];
  p_sero_safe = fmin(1 - 1e-9, fmax(1e-9, p_state_sero_at_anchor));
  alpha_sero = p_sero_safe * kappa_sero;
  beta_sero = (1 - p_sero_safe) * kappa_sero;
}

model {
  alpha_R ~ normal(log(1.2), 0.5);
  z_year ~ std_normal();
  beta_sin ~ normal(0, 0.25);
  beta_cos ~ normal(0, 0.25);
  phi_obs ~ gamma(2, 0.1);
  q ~ beta(16.2, 108.7);
  C ~ neg_binomial_2(expected_reported_cases, phi_obs);
  // Beta-binomial, NOT binomial(sero_n, p_state_sero_at_anchor) -- Section 7:
  // Juazeiro is a municipality, not a state-representative random sample.
  sero_pos ~ beta_binomial(sero_n, alpha_sero, beta_sero);
}

generated quantities {
  array[N] int C_pred;
  vector[N] log_lik_cases;
  real log_lik_serology;
  int sero_pred;
  real log_prior_q; // audit-only diagnostic (Section 6): does NOT feed back into sampling
  for (t in 1:N) {
    C_pred[t] = neg_binomial_2_rng(expected_reported_cases[t], phi_obs);
    log_lik_cases[t] = neg_binomial_2_lpmf(C[t] | expected_reported_cases[t], phi_obs);
  }
  log_lik_serology = beta_binomial_lpmf(sero_pos | sero_n, alpha_sero, beta_sero);
  sero_pred = beta_binomial_rng(sero_n, alpha_sero, beta_sero);
  log_prior_q = beta_lpdf(q | 16.2, 108.7);
}

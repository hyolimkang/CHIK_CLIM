// Pernambuco v4.9 global-q: likelihood-equivalent ridge reparameterisation.
//
...
// harmonic/year-specific seasonality, one global q, and the optional
// geographic-offset serology observation layer are unchanged.
//
// Computational changes only:
// (1) alpha_R is represented by gamma_ridge plus a fixed affine rotation of
//     logit_q. The ORIGINAL independent alpha_R and logit_q priors are applied
//     to alpha_R and logit_q, so this one-to-one transform has unit Jacobian
//     and does not alter the posterior.
// (2) the centred annual effects are represented in a Y-1 orthonormal,
//     sum-to-zero basis. This is exactly the marginal distribution induced by
//     z_year - mean(z_year), after integrating out its unused mean direction.
...
transformed parameters {
  ...
  // Unit-Jacobian coordinate transform. The alpha_R prior below is unchanged.
  alpha_R = gamma_ridge + ridge_slope * (logit_q - logit_q_reference);
  q = inv_logit(logit_q);

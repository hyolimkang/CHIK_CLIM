# Mato Grosso Phase 1 -- Sections 9-14: HMC gate, q/S identification
# diagnostics, case PPC (overall + per-wave, early vs late epidemic), and
...
message("=== Section 9: standard HMC gate ===")
...
write_csv(hmc_by_chain, file.path(table_dir, "MT_global_q_HMC.csv"))
message("[saved] ", file.path(table_dir, "MT_global_q_HMC.csv"))
...
write_csv(q_summary, file.path(table_dir, "MT_global_q_posterior_summary.csv"))
message("[saved] ", file.path(table_dir, "MT_global_q_posterior_summary.csv"))
...
message(sprintf("q-alpha_R ridge: |cor|=%.3f -- %s", abs(cor_q_alpha),
                 if (abs(cor_q_alpha) > 0.7) "STRONG ridge, analogous to RJ (|cor|~0.7-0.84 across RJ models)" else "weaker than the RJ ridge"))
...
write_csv(checkpoint_tbl, file.path(table_dir, "MT_global_q_checkpoints.csv"))
message("[saved] ", file.path(table_dir, "MT_global_q_checkpoints.csv"))

# ================================================================
# Section 12: case PPC (weekly + annual + per-wave, early vs late)
# ================================================================
...
p_weekly_ppc <- ggplot() +
  ...
  labs(title = "MT global-q case-only model: weekly case PPC",
...
ggsave(file.path(figure_dir, "MT_global_q_case_PPC.png"), p_weekly_ppc, width = 220, height = 110, units = "mm", dpi = 300, bg = "white")
message("\n[saved] ", file.path(figure_dir, "MT_global_q_case_PPC.png"))
...
p_annual_ppc <- ggplot(annual_pred_summary, aes(year)) +
  ...
  labs(title = "MT global-q case-only model: annual case totals, observed vs predicted",
       subtitle = "Orange diamond = observed; blue point+range = posterior predictive median/95% CrI",
       x = NULL, y = "Annual total cases") + theme_v4
ggsave(file.path(figure_dir, "MT_global_q_annual_PPC.png"), p_annual_ppc, width = 180, height = 110, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "MT_global_q_annual_PPC.png"))
write_csv(annual_pred_summary, file.path(table_dir, "MT_global_q_annual_PPC.csv"))

# Peak timing/magnitude check per major wave -- explicitly separating the
# early (2017-2019) epidemic from the late (2024-2025) recurrence, per
# Section 12's explicit question: can one transmission/susceptibility
# history reproduce both?
...
peak_check <- bind_rows(lapply(seq_len(nrow(mt_waves)), function(i) {
  w <- mt_waves[i, ]
  ...
  tibble(wave_id = w$wave_id, era = if (w$start_week < as.Date("2021-01-01")) "early (2017-2019)" else "late (2024-2025)",
         observed_peak_week = dates[obs_peak_idx], observed_peak_cases = weekly$cases[obs_peak_idx],
         observed_total = sum(weekly$cases[idx]), predicted_total_median = median(rowSums(draws$C_pred[, idx, drop = FALSE])),
         predicted_total_lo95 = quantile(rowSums(draws$C_pred[, idx, drop = FALSE]), .025),
         predicted_total_hi95 = quantile(rowSums(draws$C_pred[, idx, drop = FALSE]), .975),
         predicted_peak_week_median = dates[round(median(pred_peak_idx_per_draw))],
         predicted_peak_cases_median = median(apply(draws$C_pred[, idx, drop = FALSE], 1, max)))
}))
message("\n=== Per-wave peak timing/magnitude check (early vs late epidemic) ===")
print(as.data.frame(peak_check), digits = 3)
write_csv(peak_check, file.path(table_dir, "MT_global_q_wave_peak_check.csv"))

early_ok <- all(peak_check$observed_total[peak_check$era == "early (2017-2019)"] >= peak_check$predicted_total_lo95[peak_check$era == "early (2017-2019)"] &
                peak_check$observed_total[peak_check$era == "early (2017-2019)"] <= peak_check$predicted_total_hi95[peak_check$era == "early (2017-2019)"])
late_ok <- all(peak_check$observed_total[peak_check$era == "late (2024-2025)"] >= peak_check$predicted_total_lo95[peak_check$era == "late (2024-2025)"] &
               peak_check$observed_total[peak_check$era == "late (2024-2025)"] <= peak_check$predicted_total_hi95[peak_check$era == "late (2024-2025)"])
message(sprintf("\nEarly epidemic (2017-2019) totals within 95%% PPC: %s | Late recurrence (2024-2025) totals within 95%% PPC: %s",
                 early_ok, late_ok))
message(if (early_ok && late_ok) "Both epidemic eras reproduced within posterior predictive uncertainty -- no evidence the model sacrifices one period for the other."
        else "At least one epidemic era is NOT well reproduced -- inspect peak_check table for which era and by how much.")

# ================================================================
# Section 13-14: latent trajectory figure + biological plausibility
# ================================================================
...
p_S <- ggplot(S_summary, aes(week_start, median)) +
  ...
  labs(title = "S/N (case-only model-implied susceptibility)", x = NULL, y = "S / N") + theme_v4
...
p_q <- ggplot(q_band, aes(week_start)) +
  ...
  labs(title = sprintf("q_MT posterior: median=%.4f, 95%% CrI=[%.4f, %.4f]", median(q_draws), quantile(q_draws,.025), quantile(q_draws,.975)),
       x = NULL, y = "q") + theme_v4

figure <- (p_cases | p_S) / (p_U | p_R0) / p_q +
  patchwork::plot_annotation(title = "Mato Grosso: case-only model-implied latent trajectories (Phase 1, NOT validated absolute susceptibility)",
                              subtitle = sprintf("Global-q, frozen v4.9, %s", hmc_classification))
ggsave(file.path(figure_dir, "MT_global_q_trajectories.png"), figure, width = 210, height = 240, units = "mm", dpi = 300, bg = "white")
message("\n[saved] ", file.path(figure_dir, "MT_global_q_trajectories.png"))

# Section 14: biological plausibility summary
message("\n=== Section 14: biological plausibility ===")
immune_after_early <- draws$immune_prop[, which.min(abs(dates - as.Date("2019-08-18")))]
S_before_late <- draws$S_prop[, which.min(abs(dates - as.Date("2024-01-07")))]
message(sprintf("Immune fraction immediately after early epidemic (end MT_wave_06, 2019-08-18): median=%.4f [%.4f,%.4f]",
                 median(immune_after_early), quantile(immune_after_early,.025), quantile(immune_after_early,.975)))
message(sprintf("S/N immediately before late recurrence (onset MT_wave_13, 2024-01-07): median=%.4f [%.4f,%.4f]",
                 median(S_before_late), quantile(S_before_late,.025), quantile(S_before_late,.975)))
message(sprintf("Max R0(t) overall: median=%.3f [%.3f,%.3f]", median(max_R0), quantile(max_R0,.025), quantile(max_R0,.975)))
message(sprintf("Min S/N overall: median=%.4f [%.4f,%.4f]", median(min_S), quantile(min_S,.025), quantile(min_S,.975)))
message(sprintf("Cumulative infection (immune_2025): median=%.4f [%.4f,%.4f]", median(immune_2025), quantile(immune_2025,.025), quantile(immune_2025,.975)))

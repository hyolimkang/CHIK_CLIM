...
root <- project_root_v2(); source(file.path(root,"02_Script","40_susceptibility","prepare_dynamic_annual_foi_data.R"))
...
fit_annual_foi_shape_v2 <- function(){
  prepared<-build_dynamic_annual_foi_data(write_outputs=FALSE); out_t<-file.path(root,"03_Output","tables","annual_foi_shape_v2");out_f<-file.path(root,"03_Output","figures","annual_foi_shape_v2");dir.create(out_t,recursive=TRUE,showWarnings=FALSE);dir.create(out_f,recursive=TRUE,showWarnings=FALSE)
  ...
  selected<-strsplit(Sys.getenv("ANNUAL_FOI_SHAPE_V2_MODELS",paste(configs$fit_id,collapse=",")),",",fixed=TRUE)[[1]];configs<-filter(configs,fit_id%in%selected)
  shape_model<-stan_model(file.path(root,"02_Script","stan","chik_dynamic_annual_foi_shape_v2.stan"));nb_model<-stan_model(file.path(root,"02_Script","stan","chik_dynamic_annual_foi_nb_v2.stan"))

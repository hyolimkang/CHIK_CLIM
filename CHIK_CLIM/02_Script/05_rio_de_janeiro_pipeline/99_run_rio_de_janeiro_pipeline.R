# ============================================================
# RIO DE JANEIRO PIPELINE -- TOP-LEVEL ORCHESTRATOR
# ============================================================
...
# which case RIO_DE_JANEIRO_ALLOW_REFIT is set automatically -- see Stage
# 02's own runner for per-fit reuse-if-exists logic). Stages 04-09 do not
# exist for Rio de Janeiro (no sensitivity/spatial/age/vaccine/
# counterfactual/impact work was done here; state AND city are both
# inside this one pipeline -- there is no separate city folder).
...
STAGE_FOLDERS <- c(
  ...
  "04" = NA, "05" = NA, "06" = NA, "07" = NA, "08" = NA, "09" = NA # not applicable to Rio de Janeiro
)
PIPELINE_DIR <- project_path("02_Script/05_rio_de_janeiro_pipeline")

if (RUN_FROM_STAGE <= 2L && RUN_TO_STAGE >= 2L) {
  Sys.setenv(RIO_DE_JANEIRO_ALLOW_REFIT = "TRUE")
  message("[pipeline] Stage 02 is in range -- Stan refitting is ALLOWED for any missing fit (existing fits are still reused).")
} else {
  Sys.setenv(RIO_DE_JANEIRO_ALLOW_REFIT = "FALSE")
}

message(sprintf("[pipeline] Running Rio de Janeiro pipeline, Stage %02d -> Stage %02d\n", RUN_FROM_STAGE, RUN_TO_STAGE))
for (stage_num in RUN_FROM_STAGE:RUN_TO_STAGE) {
  key <- sprintf("%02d", stage_num)
  stage_folder <- STAGE_FOLDERS[[key]]
  if (is.na(stage_folder)) { message(sprintf("STAGE %s: (not applicable to Rio de Janeiro -- skipped)", key)); next }

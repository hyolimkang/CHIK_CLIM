# ===========================================================================
# 10f_build_mato_grosso_weekly_demography.R
#
# Mato Grosso equivalent of 10d_build_rio_de_janeiro_weekly_demography.R.
# Reuses the exact same generic interpolation/reconciliation logic (copied
# verbatim, not reimplemented) with UF_CODE=51 (Mato Grosso) and the MT
# births file from 09f. Does NOT modify the Ceara/Bahia/Pernambuco/Rio de
# Janeiro scripts or outputs.
#
# Output
# ------
#   01_Data/mato_grosso_weekly_demography_2015_2025.rds (+ .csv)
# ===========================================================================
...
if (sys.nframe() == 0) {
  UF_CODE <- 51L # Mato Grosso

  demography <- readRDS(here::here("01_Data/ibge_population_projection_uf_2024revision.rds")) |>
    dplyr::filter(uf_code == UF_CODE)

  births_weekly <- readRDS(here::here("01_Data/mato_grosso_sinasc_births_weekly.rds"))
  incomplete <- births_weekly |> dplyr::filter(n_days != 7)
  if (nrow(incomplete) > 0) {
    message(sprintf(
      "[drop] %d incomplete boundary week(s) from mato_grosso_sinasc_births_weekly.rds: %s",
  ...
  out_path <- here::here("01_Data/mato_grosso_weekly_demography_2015_2025.rds")

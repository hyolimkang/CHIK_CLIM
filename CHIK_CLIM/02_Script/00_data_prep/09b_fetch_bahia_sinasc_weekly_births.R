# ===========================================================================
# 09b_fetch_bahia_sinasc_weekly_births.R
#
# Bahia equivalent of 09_fetch_ce_sinasc_weekly_births.R, for the Bahia
# external-replication of the frozen Ceara v4.9 renewal model
# (BAHIA_V4_9_FROZEN_MODEL_ASSESSMENT.md). Does NOT modify the Ceara
# script or its outputs. SINASC_<year>_csv.zip files are NATIONAL (all UFs
# in one file), already cached under 01_Data/sinasc_cache/ from the Ceara
# fetch -- this script reuses that same cache and only changes the
# CODMUNRES filter from "23" (Ceara) to "29" (Bahia).
#
# Output
# ------
#   01_Data/bahia_sinasc_births_daily.rds
#   01_Data/bahia_sinasc_births_weekly.rds
# ===========================================================================
...
fetch_uf_births_one_year <- function(year, cache_dir, uf_prefix) {
  ...
  if (n_unparsed > 0) {
    message(sprintf(
      "[warn] %d/%d Bahia SINASC %d records had an unparseable DTNASC and were dropped",
...
if (sys.nframe() == 0) {
  ...
  daily_path <- here::here("01_Data/bahia_sinasc_births_daily.rds")
  weekly_path <- here::here("01_Data/bahia_sinasc_births_weekly.rds")

# ============================================================
# BAHIA PIPELINE -- TOP-LEVEL ORCHESTRATOR
# ============================================================
# ORCHESTRATOR ONLY. Safe default: RUN_FROM_STAGE=3, RUN_TO_STAGE=5 (never
# touches Stage 02 Stan fitting unless explicitly included in range, in
# which case BAHIA_ALLOW_REFIT is set automatically -- see Stage 02's own
# runner for per-fit reuse-if-exists logic). Stages 06-09 do not exist for
# Bahia (no age/vaccine/counterfactual/impact work was done here).
...
STAGE_FOLDERS <- c(
  ...
  "06" = NA, "07" = NA, "08" = NA, "09" = NA # not applicable to Bahia
)
PIPELINE_DIR <- project_path("02_Script/03_bahia_pipeline")

if (RUN_FROM_STAGE <= 2L && RUN_TO_STAGE >= 2L) {
  Sys.setenv(BAHIA_ALLOW_REFIT = "TRUE")
  message("[pipeline] Stage 02 is in range -- Stan refitting is ALLOWED for any missing fit (existing fits are still reused).")
} else {
  Sys.setenv(BAHIA_ALLOW_REFIT = "FALSE")
}

message(sprintf("[pipeline] Running Bahia pipeline, Stage %02d -> Stage %02d\n", RUN_FROM_STAGE, RUN_TO_STAGE))
for (stage_num in RUN_FROM_STAGE:RUN_TO_STAGE) {
  key <- sprintf("%02d", stage_num)
  stage_folder <- STAGE_FOLDERS[[key]]
  if (is.na(stage_folder)) { message(sprintf("STAGE %s: (not applicable to Bahia -- skipped)", key)); next }

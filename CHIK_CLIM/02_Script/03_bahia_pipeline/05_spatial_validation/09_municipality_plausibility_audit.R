...
cum_incidence <- bind_rows(lapply(checkpoints, function(cp) {
  up_to <- bahia_panel |> dplyr::filter(week_start <= cp)
  pop_at_cp <- bahia_panel |> dplyr::filter(week_start == max(week_start[week_start <= cp])) |>
...
print(as.data.frame(summary_tbl |> dplyr::filter(checkpoint == max(checkpoints))), digits = 3)
...
if (nrow(flagged) > 0) {
  flagged_summary <- flagged |> dplyr::filter(q == 0.136) |> arrange(desc(A_i)) |> select(muni6, name_muni, checkpoint, cumulative_cases, population_at_checkpoint, cumulative_reported_incidence, A_i)

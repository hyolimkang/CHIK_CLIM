...
poly <- readRDS(file.path(root, "01_Data/ibge_muni_polygons.rds")) |> dplyr::filter(substr(muni6, 1, 2) == "29")
...
for (q_val in target_qs) {
  df <- implied |> dplyr::filter(checkpoint == target_checkpoint, q == q_val)
...
flag_df <- implied |> dplyr::filter(checkpoint == target_checkpoint, q %in% target_qs) |>

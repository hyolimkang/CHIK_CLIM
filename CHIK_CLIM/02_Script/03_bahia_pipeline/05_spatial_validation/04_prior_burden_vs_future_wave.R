# Bahia spatial-turnover diagnostic -- Section 5.
#
...
# Exclude the first wave (BA_wave_02) since it has no meaningful prior
# cumulative incidence within the 2015-2025 window (near-zero for all munis).
analysis_waves <- waves$wave_id[-1]

quintile_df <- burden |>
  dplyr::filter(wave_id %in% analysis_waves) |>
  dplyr::filter(!is.na(cumulative_previous_incidence_iw)) |>
  group_by(wave_id) |>
...
write_csv(quintile_summary, file.path(table_dir, "bahia_prior_burden_future_wave.csv"))
message("\n[saved] ", file.path(table_dir, "bahia_prior_burden_future_wave.csv"))
...
write_csv(per_wave_rho, file.path(table_dir, "bahia_prior_burden_per_wave_spearman.csv"))
...
p_scatter <- ggplot(quintile_df, aes(cumulative_previous_incidence_iw + 1, incidence_iw + 1)) +
  ...
  labs(title = "Bahia: cumulative prior incidence vs subsequent-wave incidence (municipality level)",
...
ggsave(file.path(figure_dir, "bahia_prior_burden_vs_future_incidence.png"), p_scatter, width = 260, height = 200, units = "mm", dpi = 300)
message("[saved] ", file.path(figure_dir, "bahia_prior_burden_vs_future_incidence.png"))
...
ggsave(file.path(figure_dir, "bahia_prior_burden_quintile_summary.png"), p_quintile, width = 260, height = 200, units = "mm", dpi = 300)
message("[saved] ", file.path(figure_dir, "bahia_prior_burden_quintile_summary.png"))

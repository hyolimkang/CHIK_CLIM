# ===========================================================================
# 10d_build_rio_de_janeiro_weekly_demography.R
#
# Rio de Janeiro equivalent of 10c_build_pernambuco_weekly_demography.R.
# Reuses the exact same generic interpolation/reconciliation logic (copied
# verbatim, not reimplemented) with UF_CODE=33 (Rio de Janeiro) and the RJ
# births file from 09d. Does NOT modify the Ceara/Bahia/Pernambuco scripts
# or outputs.
#
# Output
# ------
#   01_Data/rio_de_janeiro_weekly_demography_2015_2025.rds (+ .csv)
# ===========================================================================
...
if (sys.nframe() == 0) {
  UF_CODE <- 33L # Rio de Janeiro

  demography <- readRDS(here::here("01_Data/ibge_population_projection_uf_2024revision.rds")) |>
    dplyr::filter(uf_code == UF_CODE)

  births_weekly <- readRDS(here::here("01_Data/rio_de_janeiro_sinasc_births_weekly.rds"))
  incomplete <- births_weekly |> dplyr::filter(n_days != 7)
  if (nrow(incomplete) > 0) {
    message(sprintf(
      "[drop] %d incomplete boundary week(s) from rio_de_janeiro_sinasc_births_weekly.rds: %s",
  ...
  out_path <- here::here("01_Data/rio_de_janeiro_weekly_demography_2015_2025.rds")

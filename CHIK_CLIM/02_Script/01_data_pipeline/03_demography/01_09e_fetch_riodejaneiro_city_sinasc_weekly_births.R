# ===========================================================================
# 09e_fetch_riodejaneiro_city_sinasc_weekly_births.R
#
# Rio de Janeiro CITY (municipality, muni6=330455) equivalent of
# 09d_fetch_rio_de_janeiro_sinasc_weekly_births.R (which filtered by UF
# prefix "33" for the whole STATE). This filters by the exact 6-digit
# municipality code instead. Reuses the same national SINASC cache.
#
# Output
# ------
#   01_Data/riodejaneiro_city_sinasc_births_daily.rds
#   01_Data/riodejaneiro_city_sinasc_births_weekly.rds
# ===========================================================================
...
MUNI6 <- "330455" # Rio de Janeiro CITY (muni6, confirmed against ibge_muni_name_lookup.rds)

fetch_muni_births_one_year <- function(year, cache_dir, muni6) {
  ...
  message("[read] ", csv_name, " (national file; filtering to muni6=", muni6, ")")
  dt <- data.table::fread(
    cmd = extract_cmd,
  ...
  if (n_unparsed > 0) {
    message(sprintf(
      "[warn] %d/%d Rio de Janeiro CITY SINASC %d records had an unparseable DTNASC and were dropped",
...
if (sys.nframe() == 0) {
  ...
  daily <- purrr::map_dfr(YEAR_START:YEAR_END, fetch_muni_births_one_year, cache_dir = cache_dir, muni6 = MUNI6)
  ...
  daily_path <- here::here("01_Data/riodejaneiro_city_sinasc_births_daily.rds")
  weekly_path <- here::here("01_Data/riodejaneiro_city_sinasc_births_weekly.rds")

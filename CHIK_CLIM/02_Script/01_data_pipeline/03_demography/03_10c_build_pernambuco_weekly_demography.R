# ===========================================================================
# 10c_build_pernambuco_weekly_demography.R
#
# Pernambuco equivalent of 10b_build_bahia_weekly_demography.R. Reuses the
# exact same generic interpolation/reconciliation logic (copied verbatim,
# not reimplemented) with UF_CODE=26 (Pernambuco) and the Pernambuco births
# file from 09c. Does NOT modify the Ceara or Bahia scripts or outputs.
#
# Output
# ------
#   01_Data/pernambuco_weekly_demography_2015_2025.rds (+ .csv)
# ===========================================================================
...
if (sys.nframe() == 0) {
  UF_CODE <- 26L # Pernambuco

  demography <- readRDS(here::here("01_Data/ibge_population_projection_uf_2024revision.rds")) |>
    dplyr::filter(uf_code == UF_CODE)

  births_weekly <- readRDS(here::here("01_Data/pernambuco_sinasc_births_weekly.rds"))
  incomplete <- births_weekly |> dplyr::filter(n_days != 7)
  if (nrow(incomplete) > 0) {
    message(sprintf(
      "[drop] %d incomplete boundary week(s) from pernambuco_sinasc_births_weekly.rds: %s",
  ...
  out_path <- here::here("01_Data/pernambuco_weekly_demography_2015_2025.rds")

# Pernambuco spatial-turnover diagnostic -- Section 1.
#
...
# panel for Pernambuco, and confirm that summing it up reproduces the exact
# PE state-level weekly case series already used by the renewal model
# (build_state_weekly(panel, "26")). Mirrors
# 31_bahia_spatial_turnover_diagnostic/scripts/01_build_bahia_municipality_week_panel.R.
...
DATE_END <- as.Date("2025-12-21") # matches the Bahia/Ceara analysis window (573 weeks)
...
pe_panel <- panel |>
  dplyr::filter(substr(muni6, 1, 2) == "26", week_start >= DATE_START, week_start <= DATE_END) |>
...
if (any(is.na(pe_panel$name_muni))) {
  stop("Some PE muni6 codes in the panel have no name-lookup match -- investigate before proceeding.")
}

saveRDS(pe_panel, file.path(table_dir, "pe_municipality_week_panel.rds"))

# ---- 2. Audit ----
n_munis <- length(unique(pe_panel$muni6))
weeks_expected <- seq(DATE_START, DATE_END, by = "week")
n_weeks_expected <- length(weeks_expected)

per_muni_week_count <- pe_panel |> count(muni6, name = "n_weeks")
incomplete_munis <- per_muni_week_count |> dplyr::filter(n_weeks != n_weeks_expected)

zero_vs_missing <- pe_panel |>
  summarise(
    n_muni_week_rows = n(),
...
#         PE renewal model (build_state_weekly, same panel/window).
official_state_weekly <- build_state_weekly(panel, "26") |>
...
audit <- tibble(
  ...
  total_pe_cases_2015_2025 = sum(pe_panel$cases_confirmed, na.rm = TRUE),
  population_coverage_min = min(pe_panel$population, na.rm = TRUE),
  population_coverage_max = max(pe_panel$population, na.rm = TRUE),
...
message("=== PE municipality-week panel audit ===")
...
write_csv(audit, file.path(table_dir, "pe_municipality_week_panel_audit.csv"))
write_csv(incomplete_munis, file.path(table_dir, "pe_municipality_incomplete_weeks.csv"))
message("\n[saved] ", file.path(table_dir, "pe_municipality_week_panel.rds"))
message("[saved] ", file.path(table_dir, "pe_municipality_week_panel_audit.csv"))

if (max_abs_diff > 0) {
  stop("STOP: municipality-level aggregation does NOT exactly reproduce the state-level series used by the PE renewal model. Investigate before proceeding to wave burden analysis.")
} else {
  message("\nCONFIRMED: summing the municipality-week panel reproduces the exact state-level case series used by the PE renewal model (max abs diff = 0 across ", nrow(reconciled), " weeks; n_municipalities = ", n_munis, ").")
}

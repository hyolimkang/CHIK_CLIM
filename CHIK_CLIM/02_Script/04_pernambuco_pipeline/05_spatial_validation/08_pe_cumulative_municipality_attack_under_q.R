...
cum_incidence <- bind_rows(lapply(checkpoints, function(cp) {
  up_to <- pe_panel |> dplyr::filter(week_start <= cp)
  pop_at_cp <- pe_panel |> dplyr::filter(week_start == max(week_start[week_start <= cp])) |>
...
print(as.data.frame(summary_tbl |> dplyr::filter(checkpoint == max(checkpoints)) |>
...
# ---- Flag A_i > 1 -----------------------------------------------------------
flagged <- implied |> dplyr::filter(A_i > 1) |>
...
if (nrow(flagged) > 0) {
  flagged_at_median_q <- flagged |> dplyr::filter(q_label == "global-q median") |> arrange(desc(A_i)) |>
...
poly <- readRDS(file.path(root, "01_Data/ibge_muni_polygons.rds")) |> dplyr::filter(substr(muni6, 1, 2) == "26")
...
map_df <- implied |> dplyr::filter(checkpoint == target_checkpoint, q_label %in% map_labels) |>
...
flag_df <- implied |> dplyr::filter(checkpoint == target_checkpoint, q_label %in% map_labels) |>

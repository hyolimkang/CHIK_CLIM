...
lookup <- readRDS(file.path(root, "01_Data/ibge_muni_name_lookup.rds")) |> dplyr::filter(uf == "PE")
...
if (any(is.na(matched$muni6))) {
  message("Unmatched:"); print(as.data.frame(matched |> dplyr::filter(is.na(muni6))))
...
print(as.data.frame(stratum_lookup |> dplyr::filter(is.na(population)) |> select(muni6, municipality_name, geres, final_model_stratum)))

# Pernambuco external replication -- established-convention 6/7-panel
# trajectory figure (same layout/theme as
# 30_bahia_v4_9_replication/scripts/15_plot_bahia_global_q_seven_panel.R).
# Model A (case-only, no serology) gets 6 panels; Model B (U14 serology)
# gets 7 panels with the serology comparison panel G.
...
plot_pe_panel <- function(tag, stage = "pilot") {
  ...
  p_cases <- ggplot() +
    ...
    x_scale +
    labs(title = "Reported cases and posterior prediction", subtitle = "No seed mechanism used (no PE gap qualifies)",
  ...
  # Prior is deliberately very broad (95% CI on the probability scale spans
  # roughly 1.5%-45%) -- it exists only to avoid pre-identifying q, not to
  # be plotted alongside the (much narrower) posterior, so it is computed
  # here only for the subtitle text, not drawn as a ribbon/line.
  ...
  p_ascertainment <- ggplot(q_band, aes(week_start)) +
    ...
    x_scale +
    labs(title = "Case ascertainment (q)", subtitle = sprintf("q ESTIMATED: posterior median=%.3f, 95%% CrI=[%.3f, %.3f] (prior: broad, logit_q~N(logit(0.10),1), not shown)",
  ...
  if (has_serology) {
    # Observed U14 (Recife) point overlaid on the STATE-level trajectory,
    # with a VERTICAL error bar showing the reported 95% CI of the
    # prevalence estimate (0.340-0.404, per the design spec) -- same
    # convention as the Ceara fixed-model serology panel. This is
    # deliberately NOT expected to sit on the state-level (no geo offset)
    # curve: Recife is PE's capital and is allowed a different local
    # prevalence via eta_geo (panel G shows the geo-adjusted comparison,
    # which IS what is actually fit to this observation).
    audit <- bundle$sero_audit
    sero_point_df <- tibble::tibble(
      week_start = audit$first_week + as.numeric(audit$last_week - audit$first_week) / 2,
      observed_prevalence = audit$observed_prevalence,
      observed_lo95 = 0.340, observed_hi95 = 0.404,
      sero_id = audit$sero_id
    )
    p_attack <- p_attack +
      geom_errorbar(data = sero_point_df,
                     aes(week_start, ymin = observed_lo95, ymax = observed_hi95),
                     width = 60, colour = COL_INK, linewidth = 0.4, inherit.aes = FALSE) +
      geom_point(data = sero_point_df, aes(week_start, observed_prevalence, shape = "Observed U14 (Recife, raw)"),
                 colour = COL_INK, size = 2.2, inherit.aes = FALSE) +
      scale_shape_manual(values = c("Observed U14 (Recife, raw)" = 18), name = NULL) +
      labs(subtitle = "Black diamond = observed Recife U14 prevalence (reported 95% CI). NOT expected to match the state curve --\nRecife's local prevalence is allowed to differ via eta_geo; see panel G for the geo-adjusted comparison that IS fit to this point.") +
      theme(legend.position = "bottom")
  }

  subtitle_model <- sprintf("Frozen v4.9 + global q ESTIMATED%s (%s, %s)",
                             if (has_serology) " + U14 Recife serology (geo-adjusted)" else " (case-only control)",
  ...
  if (has_serology) {
    ...
    sero_df <- bind_rows(lapply(seq_len(nrow(audit)), function(j) {
      tibble::tibble(
        ...
        quantity = c("Observed", "PE state-level (no geo offset)", "Geo-adjusted Recife"),
    ...
    p_sero <- ggplot(sero_df, aes(sero_id, median, colour = quantity)) +
      ...
      labs(title = "U14 Recife serosurvey: observed vs model (global q)", x = "survey", y = "seroprevalence") +
    ...
    figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
      patchwork::plot_annotation(title = "Chikungunya transmission in Pernambuco, Brazil (2015-2025) -- case-only control",
                                  subtitle = subtitle_model, tag_levels = "A",
                                  theme = theme(plot.title = element_text(size = 12.5, face = "bold", margin = margin(b = 3)),
                                                plot.subtitle = element_text(size = 9.5, colour = COL_MUTED, margin = margin(b = 8)),
                                                plot.tag = element_text(size = 11, face = "bold")))
  ...
  output_base <- paste0("pernambuco_trajectories_", tag)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("pernambuco_trajectory_summary_", tag, ".csv"))
...
if (sys.nframe() == 0L) {
  tag <- Sys.getenv("TAG", "modelA_caseonly")
  stage <- Sys.getenv("STAGE", "pilot")
  plot_pe_panel(tag, stage)
}

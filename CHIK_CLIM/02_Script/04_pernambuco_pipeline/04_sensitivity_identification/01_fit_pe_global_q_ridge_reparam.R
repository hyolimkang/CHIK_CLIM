# Pernambuco v4.9 global-q ridge-reparameterised fit (Model C).
#
# This is a computationally improved coordinate system for the frozen v4.9
# model, not a new epidemiological model. See PE_V4_9_GLOBAL_Q_RIDGE_REPARAM.md.
...
# Numerical preconditioner estimated from the prior PE case-only pilot. It is
# fixed before the Model-C production fit and has no effect on the posterior.
RIDGE_SLOPE <- -0.45
...
make_pe_ridge_data <- function(weekly, sero_on = TRUE) {
  ...
  prepared$stan_data$ridge_slope <- RIDGE_SLOPE
  prepared$stan_data$logit_q_reference <- LOGIT_Q_PRIOR_MEAN
...
make_pe_ridge_init_fn <- function(Y, J_sero, scalar_inits) {
  q_starts <- c(0.006, 0.010, 0.020, 0.050)
  offsets <- c(-0.03, -0.01, 0.01, 0.03)
  function(chain_id = 1L) {
    idx <- (as.integer(chain_id) - 1L) %% 4L + 1L
    base <- scalar_inits[[idx]]
    list(
      gamma_ridge = -0.66 + offsets[idx],
...
run_pe_v4_9_global_q_ridge_reparam <- function(
  ...
  hmc <- compute_hmc_gate(fit,
    c("gamma_ridge", "alpha_R", "z_year_contrast", "beta_sin1", "beta_cos1", "beta_sin2", "beta_cos2", "sigma_season_year", "z_season_year", "phi_obs", "logit_q"),
    settings$max_treedepth
  )
  tag <- if (sero_on) "modelC_U14" else "modelC_caseonly"
  bundle <- list(
    fit = fit, stan_data = prepared$stan_data, weekly_data = weekly, sero_audit = prepared$sero_audit,
    config = c(settings, list(model_version = "pernambuco_v4_9_global_q_ridge_reparam", tag = tag,
      stage = stage, sero_on = sero_on, ridge_slope = RIDGE_SLOPE,
      logit_q_reference = LOGIT_Q_PRIOR_MEAN, q_prior_mean = 0.10,
      q_prior_logit_sd = LOGIT_Q_PRIOR_SD, elapsed_seconds = elapsed_seconds,
      likelihood_note = "Same v4.9 likelihood and priors; computational coordinate transform only.")),
    hmc = hmc
  )
  ad_suffix <- if (settings$adapt_delta != 0.95) paste0("_ad", gsub("\\.", "", sprintf("%.2f", settings$adapt_delta))) else ""
  fit_path <- file.path(out_dir, paste0("pe_ridge_", if (sero_on) "U14" else "caseonly", "_", stage, ad_suffix, ".rds"))
  saveRDS(bundle, fit_path)
  saveRDS(prepared$stan_data, file.path(out_dir, paste0("stan_data_", tag, "_", stage, ".rds")))
...
if (sys.nframe() == 0L) run_pe_v4_9_global_q_ridge_reparam()

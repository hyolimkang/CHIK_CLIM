...
sero_label <- if (grepl("U14", tag)) "U14" else "caseonly" # matches 01_fit_pe_global_q_ridge_reparam.R's actual fit_path naming (the short "outputs/modelC" dir was adopted after a Windows MAX_PATH failure under the older "modelC_global_q_ridge_reparam" out_dir, which now holds only raw, never-combined chain CSVs)
out_dir <- file.path(root, "03_Output", "04_pernambuco_pipeline", "model_fits", "v4_9_replication", "outputs", "modelC")
table_dir <- file.path(root, "03_Output", "04_pernambuco_pipeline", "tables", "pernambuco_v4_9_ridge_reparam")
fit_path <- file.path(out_dir, paste0("pe_ridge_", sero_label, "_", stage, ".rds"))

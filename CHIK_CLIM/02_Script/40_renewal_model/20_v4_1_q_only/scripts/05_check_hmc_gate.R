# Post-hoc strict HMC gate check for one completed v4.1 q-only stage fit.
...
# every chain.
#
# Usage: STAGE=B Rscript 05_check_hmc_gate.R
...
run_check_hmc_gate <- function(stage = Sys.getenv("STAGE", "B")) {
  root <- gate_root()
  out_dir <- file.path(root, "02_Script", "40_renewal_model", "20_v4_1_q_only", "outputs", paste0("stage", stage))

# Live, read-only monitor for a running v4.2 (q + Juazeiro serology) stage
# fit. Run in a SEPARATE terminal while 02_fit_v4_2.R is sampling:
#   STAGE=B Rscript 03_monitor_v4_2.R
#
...
run_monitor_v4_2 <- function(stage = Sys.getenv("STAGE", "B"), poll_seconds = 60, max_polls = Inf) {
  root <- monitor_root()
  base_dir <- file.path(root, "02_Script", "40_renewal_model", "21_v4_2_q_juazeiro_serology")
  ...
  stage_warmup <- switch(stage, A = 5L, B = 300L, B2 = 400L, B4 = 400L, C = 500L, D = 1000L)
  stage_sampling <- switch(stage, A = 5L, B = 150L, B2 = 200L, B4 = 200L, C = 300L, D = 1000L)
  chains <- 4L
...
if (sys.nframe() == 0L) run_monitor_v4_2()

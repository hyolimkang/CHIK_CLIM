# Pernambuco spatial-turnover diagnostic -- Section 6.
#
...
p_maps <- ggplot(map_data) +
  ...
  labs(title = "Pernambuco municipality-level reported incidence by major epidemic wave",
       subtitle = sprintf("Same colour scale across waves (capped at %.0f/100k, 98th percentile) -- no allfoi used", incidence_cap)) +
  theme_map

ggsave(file.path(figure_dir, "pe_wave_incidence_maps.png"), p_maps, width = 260, height = 300, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "pe_wave_incidence_maps.png"))
...
p_max <- ggplot(map_max) +
  geom_sf(aes(fill = wave_id_max), colour = "white", linewidth = 0.08) +
  scale_fill_viridis_d(option = "turbo", name = "Wave with max\nincidence", na.value = "grey90") +
  labs(title = "Pernambuco: which epidemic wave produced each municipality's peak incidence",
...
ggsave(file.path(figure_dir, "pe_wave_max_incidence_summary_map.png"), p_max, width = 200, height = 180, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "pe_wave_max_incidence_summary_map.png"))

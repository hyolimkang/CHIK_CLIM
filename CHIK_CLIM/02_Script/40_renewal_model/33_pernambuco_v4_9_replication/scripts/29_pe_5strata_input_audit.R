...
stratum_pop_2025 <- weekly_stratum |> filter(week_start == max(week_start)) |> select(final_model_stratum, population)
...
wave_contrib <- bind_rows(lapply(seq_len(nrow(waves)), function(i) {
  w <- waves[i, ]
  panel_stratum |>
    filter(week_start >= w$start_week, week_start <= w$end_week) |>
...
for (wid in names(key_waves)) {
  ...
  print(as.data.frame(wave_contrib |> filter(wave_id == wid) |>
...
for (wid in names(key_waves)) {
  sub <- wave_contrib |> filter(wave_id == wid) |> arrange(desc(share_of_wave))
...
writeLines(md_lines, file.path(pe_root, "PE_5strata_input_audit.md"))
message("\n[saved] ", file.path(pe_root, "PE_5strata_input_audit.md"))

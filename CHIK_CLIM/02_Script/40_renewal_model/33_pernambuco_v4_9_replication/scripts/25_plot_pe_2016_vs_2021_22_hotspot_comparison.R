...
print(as.data.frame(waves |> filter(wave_id %in% c(WAVE_2016, WAVES_2021_22)) |>
...
pooled_2021_22 <- burden |>
  filter(wave_id %in% WAVES_2021_22) |>
...
wave_2016 <- burden |> filter(wave_id == WAVE_2016) |>
  select(muni6, name_muni, reported_cases_iw, population_mean_iw, incidence_iw, wave_id)

three_panel <- bind_rows(
  burden |> filter(wave_id %in% WAVES_2021_22) |> select(muni6, wave_id, incidence_iw),

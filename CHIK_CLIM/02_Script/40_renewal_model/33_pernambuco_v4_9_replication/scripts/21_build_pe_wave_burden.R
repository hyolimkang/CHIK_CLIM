...
pe_waves <- wave_census |>
  filter(state == "PE", major_epidemic_primary == TRUE) |>
  transmute(
    wave_id, wave_order,
...
compute_wave_burden <- function(wave_row, panel) {
  in_wave <- panel |> filter(week_start >= wave_row$start_week, week_start <= wave_row$end_week)
  before_wave <- panel |> filter(week_start < wave_row$start_week)

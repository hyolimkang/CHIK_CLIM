...
STAGE_DIR <- file.path(project_path("02_Script/40_renewal_model/38_ceara_current_pipeline"), "08_impact_nnv_summaries")
...
qa_summary_path <- project_path("02_Script/40_renewal_model/37_ceara_historical_age12_vaccine_counterfactual/diagnostics/QA_overall_summary.csv")
if (!file.exists(qa_summary_path)) {
  stop("Missing prerequisite (three-arm result has not been validated):\n  ", qa_summary_path,
       "\n\nRun Stage 07 first:\n  07_vaccine_validation (00_run_stage_07.R)", call. = FALSE)
}

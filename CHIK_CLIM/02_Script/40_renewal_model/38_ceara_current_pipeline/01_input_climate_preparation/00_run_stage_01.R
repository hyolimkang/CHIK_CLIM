...
STAGE_DIR <- file.path(project_path("02_Script/40_renewal_model/38_ceara_current_pipeline"), "01_input_climate_preparation")
...
if (!file.exists(national_climate_path)) {
  stop("Missing prerequisite:\n  ", national_climate_path,
       "\n\nThis is built by the national wave analysis track (02_Script/40_renewal_model/15_national_wave_analysis/), not by this stage.", call. = FALSE)
}

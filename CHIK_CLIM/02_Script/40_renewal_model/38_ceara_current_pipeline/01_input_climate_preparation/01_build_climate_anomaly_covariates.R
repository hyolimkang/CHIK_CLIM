...
climatology <- lagged_all |> filter(!is.na(lagged_temp), !is.na(log_precip)) |>
...
scaling <- anomalies |> filter(state %in% PRIMARY_STATES, !is.na(T_anom), !is.na(P_anom)) |>
  group_by(lag_spec) |>
...
qc_pooled <- anomalies |> filter(state %in% PRIMARY_STATES) |> group_by(lag_spec) |>
...
qc_seasonal <- anomalies |> filter(lag_spec == "primary", state %in% PRIMARY_STATES, !is.na(T_anom), !is.na(P_anom)) |>
  group_by(state) |>
...
# ================================================================
qc_data <- anomalies |> filter(lag_spec == "primary", state %in% PRIMARY_STATES)

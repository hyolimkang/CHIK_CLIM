...
STAGE_DIR <- file.path(PIPELINE_DIR, "02_historical_transmission_fitting")
...
if (!file.exists(climate_covariates_path)) {
  stop("Missing prerequisite:\n  ", climate_covariates_path,
       "\n\nRun Stage 01 first:\n  01_input_climate_preparation (00_run_stage_01.R)", call. = FALSE)
}
...
if (file.exists(td14_path)) {
  ...
  source(project_path("02_Script/40_renewal_model/17_v4_0_minimal_no_vaccine/01_fit_v4_0_minimal_no_vaccine.R"), local = .GlobalEnv) # defines run_v4_0_minimal_no_vaccine(); does NOT auto-run (sys.nframe() != 0 under source())
...
v49_baseline_path <- project_path("02_Script/40_renewal_model/28_v4_9_hierarchical_seasonality/outputs/q0.05/renewal_ceara_v4_9_fit_q0.05.rds")
if (file.exists(v49_baseline_path)) {
  ...
  source(project_path("02_Script/40_renewal_model/28_v4_9_hierarchical_seasonality/scripts/01_fit_v4_9.R"), local = .GlobalEnv) # defines run_fit_v4_9(); does NOT auto-run

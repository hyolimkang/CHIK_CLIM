...
decomp <- read_csv(file.path(DIR_TABLES, "TABLE3_effect_decomposition.csv"), show_col_types = FALSE) |> filter(quantity != "Indirect fraction of total effect")
...
cohort_example <- vax |> filter(doses_B > 1e-6) |> mutate(year = as.integer(format(week_start, "%Y")), birth_cohort = year - TARGET_AGE) |>

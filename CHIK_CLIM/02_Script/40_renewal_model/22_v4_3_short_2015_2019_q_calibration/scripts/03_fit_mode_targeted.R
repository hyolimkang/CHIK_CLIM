# Mode audit (Section 1): re-fits the EXACT Fit B scientific model (cases +
# Juazeiro, 2015-2019, weak eta_q prior) and dense_e sampler configuration,
# but with ALL 4 chains dispersed WITHIN one target region instead of
# spanning the whole prior. This does NOT change the model, prior,
# kappa_sero, or sampler settings -- only where chains start, to
# characterize each local posterior (not to pick a winner by fiat).
#
# Usage: MODE=low  Rscript 03_fit_mode_targeted.R   (q inits ~0.04-0.07)
#        MODE=high Rscript 03_fit_mode_targeted.R   (q inits ~0.75-0.90)
...
base_dir <- file.path(root, "02_Script", "40_renewal_model", "22_v4_3_short_2015_2019_q_calibration")

mode_q_ranges <- list(low = c(0.04, 0.05, 0.06, 0.07), high = c(0.75, 0.80, 0.85, 0.90),
                      lowclean = c(0.045, 0.05, 0.055))

run_fit_mode_targeted <- function(mode = Sys.getenv("MODE", "low")) {
  if (!mode %in% names(mode_q_ranges)) stop("MODE must be 'low', 'high', or 'lowclean'.")
  tag <- paste0("modeB_", mode)
  message("[", tag, "] mode-targeted Fit B re-fit: q inits in [", min(mode_q_ranges[[mode]]), ", ", max(mode_q_ranges[[mode]]), "]")
  ...
  prepared <- make_v4_3_data(weekly, fixed$imports_per_week, fixed$year_effect_prior_sd,
                              sero_week$index, fixed$sero_pos, fixed$sero_n, fixed$kappa_sero, use_serology = 1L)
  iter_total <- settings$warmup + settings$iter_sampling
  sample_file_base <- file.path(progress_dir, "chain")
  td14_scalar_inits <- load_td14_scalar_inits(root, n_chains = n_chains)

  q_inits <- mode_q_ranges[[mode]]
  offsets <- seq(-0.06, 0.06, length.out = n_chains)
  init_fn <- function(chain_id = 1L) {
    cid <- as.integer(chain_id)
    base <- td14_scalar_inits[[(cid - 1L) %% length(td14_scalar_inits) + 1L]]
    offset <- offsets[(cid - 1L) %% n_chains + 1L]
  ...
  bundle <- list(
    ...
    config = c(fixed, settings, list(model_version = "v4_3_mode_targeted", mode = mode, tag = tag,
...
if (sys.nframe() == 0L) run_fit_mode_targeted()

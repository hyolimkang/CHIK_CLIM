# Six-panel trajectory figure for a single v4.8 fixed-q fit, same format
# as 25_v4_6.../03_plot_v4_6_six_panel.R and 26_v4_7.../03_plot_v4_7_six_panel.R.
# The only addition: the 2022 seed/conditioning window is shaded grey in
# panels A and B, since expected_reported_cases and X there are, by
# construction, ~= the observed data (X_seed = C/q, excluded from the
# likelihood) and must not be read as predictive performance.
#
# Usage: QVAL=0.10 Rscript 03_plot_v4_8_six_panel.R
...
base_dir <- file.path(root, "02_Script/40_renewal_model/27_v4_8_seeded_recurrence")

plot_v4_8_six_panel <- function(qval) {
  tag <- paste0("q", sprintf("%.2f", qval))
  bundle <- readRDS(file.path(base_dir, "outputs", tag, paste0("renewal_ceara_v4_8_fit_", tag, ".rds")))
  fit <- bundle$fit
  ...
  p_cases <- seed_rect(ggplot()) +
    ...
    labs(title = "Reported cases and posterior prediction",
         subtitle = "Grey band = 2022 seed window (conditioned on data, excluded from likelihood -- not predictive)",
  ...
  p_infections <- seed_rect(add_interval_layers(ggplot(latent, aes(week_start)), latent, COL_VERMILLION)) +
    ...
    labs(title = "Latent infection incidence", subtitle = "Grey band = 2022 seed window (X = C/q, conditioned, not renewal-derived)",
  ...
  p_ascertainment <- ggplot(ascertainment_df, aes(week_start, q)) +
    ...
    x_scale +
    labs(title = "Case ascertainment", subtitle = sprintf("q FIXED at %.2f (not estimated -- v4.8 seeded-recurrence sweep)", qval),
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(
      ...
      subtitle = sprintf("v4.8 seeded-recurrence sweep, q = %.2f (%s) | 2022 reintroduction conditioned, 8-week seed window",
  ...
  output_base <- paste0("renewal_v4_8_trajectories_ceara_", tag)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("renewal_v4_8_trajectory_summary_", tag, ".csv"))

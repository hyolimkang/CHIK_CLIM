# Mato Grosso external replication -- Section 4-5: build the complete v4.9
# input (weekly cases + demography, same window/contract as CE/BA/PE/RJ)
# and the seed-qualification check using the SAME pre-specified long-gap
# rule -- no manual MT-specific seed decision.
...
weekly_cases <- build_state_weekly(panel, "51") |> filter(week_start >= DATE_START, week_start <= DATE_END)

demography_path <- file.path(root, "01_Data/mato_grosso_weekly_demography_2015_2025.rds")
if (!file.exists(demography_path)) stop("MT weekly demography not yet built -- run 09f/10f in 02_Script/00_data_prep first.")
weekly_demography <- readRDS(demography_path) |> filter(week_start >= DATE_START, week_start <= DATE_END)

weekly <- weekly_cases |> select(-population) |> inner_join(weekly_demography, by = "week_start") |> arrange(week_start)
if (nrow(weekly) != nrow(weekly_cases) || nrow(weekly) != nrow(weekly_demography)) stop("Panel mismatch -- MT case series and demography weeks do not align.")
...
audit <- tibble(
  state = "MT", n_weeks = nrow(weekly), n_weeks_expected = length(weeks_expected),
  ...
  importation_convention = "imports_per_week = 1 (frozen v4.9 default, same as CE/BA/PE/RJ, unchanged)",
  data_source_confirmed = "chik_dlnm_panel_muni_week_2015_2025.rds (UF prefix 51) + mato_grosso_weekly_demography_2015_2025.rds -- NOT inherited CE/BA/PE/RJ objects/paths"
)

message("=== Mato Grosso v4.9 input audit ===")
print(as.data.frame(audit))
write_csv(audit, file.path(table_dir, "mato_grosso_v49_input_audit.csv"))
message("[saved] ", file.path(table_dir, "mato_grosso_v49_input_audit.csv"))

if (!audit$week_continuity_ok || audit$n_na_any > 0 || accounting_error > 1e-6) {
  stop("STOP: MT input audit failed -- do not proceed to fitting.")
}
saveRDS(weekly, file.path(table_dir, "mato_grosso_weekly_input.rds"))
...
mt_major_waves <- wave_census |> filter(state == "MT", major_epidemic_primary == TRUE) |>
...
gaps <- mt_major_waves |>
  ...
  filter(!is.na(weeks_since_previous_wave))

message("\n=== MT major-wave gaps (seed-qualification check) ===")
...
qualifying <- gaps |> filter(weeks_since_previous_wave >= CEARA_2022_GAP_WEEKS)

message(sprintf("\nMax MT inter-wave gap = %.0f weeks (vs the %.0f-week Ceara-2022 standard). Qualifying transitions: %d.",
                 max(gaps$weeks_since_previous_wave), CEARA_2022_GAP_WEEKS, nrow(qualifying)))
if (nrow(qualifying) == 0) {
  message("CONCLUSION: No MT inter-wave gap qualifies under the established long-gap rule. NO SEED MECHANISM will be used for MT (is_seed all-zero), despite the visually 'quiet' 2019-2023 interval -- continuous low importation (frozen v4.9 default) is retained.")
} else {
  message("CONCLUSION: The following MT transition(s) QUALIFY for a conditioned seed under the established rule:")
  print(as.data.frame(qualifying))
}
write_csv(gaps, file.path(table_dir, "mato_grosso_wave_gap_seed_check.csv"))
message("[saved] ", file.path(table_dir, "mato_grosso_wave_gap_seed_check.csv"))
...
writeLines(md, file.path(root, "02_Script/40_renewal_model/35_mato_grosso_v4_9_replication/MT_DATA_AUDIT.md"))

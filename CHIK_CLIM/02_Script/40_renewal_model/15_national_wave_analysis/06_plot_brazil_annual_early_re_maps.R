...
annual_map <- function(year_value, geometry, state_year, fill_limits, show_legend = FALSE) {
  event_data <- filter(state_year, calendar_year == year_value)
  data <- geometry$municipal |> left_join(event_data, by = "state")
  state_points <- geometry$state_points |> left_join(event_data, by = "state")
  unavailable <- filter(data, event_status == "major_re_unavailable")
  first <- filter(state_points, event_status == "first_usable")
  recurrent <- filter(state_points, event_status == "recurrent_usable")
  multiple <- filter(state_points, n_major_waves > 1L)

...
source(file.path(gam_root(), "02_Script", "40_renewal_model", "15_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
leave_one_state_out <- function(data, formula) {
  ...
  for (i in seq_along(states)) {
    held_out <- states[i]
    train <- data |> filter(state != held_out) |> droplevels()
    test <- data |> filter(state == held_out)
...
model_summary_row <- function(model, model_name, data, cv_formula, climate_window, foi_scale_tag) {
  ...
  cv <- cv |> filter(is.finite(observed), is.finite(predicted))

...
state_fit_paths <- c(
  BA = file.path(root, "02_Script/40_renewal_model/30_bahia_v4_9_replication/outputs_global_q_multisite_serology/on_full2/renewal_bahia_global_q_fit_on_full2.rds"),
  RJ = file.path(root, "02_Script/40_renewal_model/34_rio_de_janeiro_v4_9_replication/outputs/caseonly/rj_global_q_case_only.rds"),
  MT = file.path(root, "02_Script/40_renewal_model/35_mato_grosso_v4_9_replication/outputs/caseonly/mt_global_q_case_only.rds")
)
...
climate_panel <- bind_rows(lapply(PRIMARY_STATES, function(state) {
  weekly_climate_summary(climate |> filter(state == !!state))
})) |>
  mutate(
...
climate_prediction_input <- climate_panel |>
  filter(!is.na(temperature_summary), !is.na(precipitation_summary))
prediction <- predict(
  gam_fit,
...
first_wave_end <- master |>
  filter(state %in% PRIMARY_STATES, wave_order == 1L) |>
  group_by(state) |>

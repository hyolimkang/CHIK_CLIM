...
recurrent <- master |> filter(state %in% c("BA", "RJ", "MT", "CE"), wave_order > 1, re6_analysis_usable) |>
...
# ================================================================
state_fits <- list(
  BA = list(path = file.path(root, "02_Script/40_renewal_model/30_bahia_v4_9_replication/outputs_global_q_multisite_serology/on_full2/renewal_bahia_global_q_fit_on_full2.rds"),
            identification_status = "serology_anchored", q_scenario = NA_character_),
  RJ = list(path = file.path(root, "02_Script/40_renewal_model/34_rio_de_janeiro_v4_9_replication/outputs/caseonly/rj_global_q_case_only.rds"),
            identification_status = "partially_identified_case_only", q_scenario = NA_character_),
  MT = list(path = file.path(root, "02_Script/40_renewal_model/35_mato_grosso_v4_9_replication/outputs/caseonly/mt_global_q_case_only.rds"),
...
for (q in CE_Q_SCENARIOS) {
  state_fits[[paste0("CE_q", q)]] <- list(
    path = file.path(root, sprintf("02_Script/40_renewal_model/28_v4_9_hierarchical_seasonality/outputs/q%s/renewal_ceara_v4_9_fit_q%s.rds", q, q)),
...
climate_pred_rows <- bind_rows(lapply(seq_len(nrow(recurrent)), function(i) {
  ep <- recurrent[i, ]
  climate_uf <- climate |> filter(state == ep$UF)
...
re_pred_rows <- bind_rows(lapply(seq_len(nrow(s_pre_tbl)), function(i) {
  s_row <- s_pre_tbl[i, ]
  c_row <- climate_pred_rows |> filter(UF == s_row$UF, episode_id == s_row$episode_id)
  if (s_row$UF == "CE") {

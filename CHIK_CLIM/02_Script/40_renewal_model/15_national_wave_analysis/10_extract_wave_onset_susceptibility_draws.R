...
source(file.path(extract_root(), "02_Script", "40_renewal_model", "15_national_wave_analysis", "00_national_wave_analysis_helpers.R"))
...
place_weekly_and_capture_onset <- function(state, annual, X, S_start, U_start, state_week, settings, major_onsets) {
  ...
  for (y_index in seq_along(settings$years)) {
    ...
    this_year_onsets <- major_onsets |> filter(state == !!state, year(onset_week) == annual_year)
    if (!nrow(this_year_onsets)) next
    population_week_start <- vapply(seq_len(nrow(year_weeks)), function(j) {
      interpolate_uf_population(read_national_demography() |> filter(state == !!state), year_weeks$week_start[j])
    }, numeric(1))
...
run_extract_wave_onset_susceptibility_draws <- function() {
  ...
  major_onsets <- census |> filter(major_epidemic_primary) |> select(state, wave_id, onset_week)

...
compute_R_survey <- function(sero_id_val) {
  audit_row <- sero_audit |> filter(sero_id == sero_id_val)
  ...
  muni_series <- bahia_panel |> filter(muni6 == muni6_val) |> arrange(week_start) |>
    mutate(cumulative_cases = cumsum(cases_confirmed), cumulative_incidence = cumulative_cases / population)

  window_incidence <- muni_series |> filter(week_start %in% window_dates) |> pull(cumulative_incidence)
...
u_block <- implied_q_tbl |> filter(sero_id %in% c("U19", "U20", "U21", "U22"))

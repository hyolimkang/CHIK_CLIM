# Six-panel trajectory figure for a single Bahia fixed-q fit, same overall
# format as 28_v4_9.../03_plot_v4_9_six_panel.R with two Bahia-specific
# differences: (1) no seed-window shading (Bahia uses no seed mechanism --
# see BAHIA_V4_9_FROZEN_MODEL_ASSESSMENT.md Section 6), (2) panel F shows
# the cumulative-infection trajectory alone, with NO serology anchor
# overlay (Bahia has no serosurvey; do not borrow Ceara's).
#
# Figures are saved INSIDE this Bahia subfolder (not 03_Output/figures/),
# per explicit instruction.
#
# Usage: QVAL=0.05 STAGE=pilot Rscript 03_plot_bahia_six_panel.R
...
plot_bahia_six_panel <- function(qval, stage = "pilot") {
  ...
  bundle <- readRDS(file.path(base_dir, "outputs", tag, paste0("renewal_bahia_v4_9_fit_", tag, ".rds")))
  fit <- bundle$fit
  ...
  p_cases <- ggplot() +
    ...
    labs(title = "Reported cases and posterior prediction",
         subtitle = "No seed mechanism used (no Bahia gap qualifies -- see assessment Sec.6)",
  ...
  p_immunity <- ggplot(immunity, aes(week_start, colour = state, fill = state)) +
    ...
    x_scale +
    labs(title = "Population susceptibility and immunity", subtitle = "Closed population; LIFELONG immunity; conditional on fixed q",
  ...
  p_reproduction <- ggplot(reproduction, aes(week_start, colour = number, fill = number)) +
    ...
    x_scale +
    labs(title = "Time-varying reproduction numbers", subtitle = "Frozen v4.9 structure: harmonic + year-specific amplitude A_year",
  ...
  p_ascertainment <- ggplot(ascertainment_df, aes(week_start, q)) +
    ...
    x_scale +
    labs(title = "Case ascertainment", subtitle = sprintf("q FIXED at %.2f (not estimated; no Bahia serology -- conditional on q)", qval),
  ...
  p_attack <- ggplot(attack, aes(week_start)) +
    ...
    x_scale +
    labs(title = "Cumulative infection (immune fraction)", subtitle = "NO serology anchor for Bahia -- not borrowed from Ceara",
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(
      title = "Chikungunya transmission and population susceptibility in Bahia, Brazil (2015-2025)",
      subtitle = sprintf("Frozen Ceara v4.9 structure, external replication, q = %.2f (%s, %s) | case likelihood only",
  ...
  output_base <- paste0("bahia_v4_9_trajectories_", tag, "_", stage)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("bahia_v4_9_trajectory_summary_", tag, "_", stage, ".csv"))
...
if (sys.nframe() == 0L) {
  ...
  plot_bahia_six_panel(qval, stage)
}

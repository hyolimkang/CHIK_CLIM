...
p_compare <- ggplot(sero_plot_df |> filter(!is.na(q)), aes(factor(q), median, colour = quantity)) +
  geom_pointrange(aes(ymin = lo, ymax = hi), position = position_dodge(width = .4), fatten = 1.8) +
  geom_hline(data = sero_plot_df |> filter(is.na(q)), aes(yintercept = median), linetype = 2, colour = "black") +

...
attach_stability <- function(primary, all_waves) {
  ...
  has_match <- function(row, definition, major_only = FALSE) {
    candidates <- filter(all_waves, segmentation_definition == definition, state == row$state)
    if (major_only) candidates <- filter(candidates, major_epidemic_primary)
...
summarise_census <- function(waves, definition, all_states) {
  major <- filter(waves, major_epidemic_primary)
  n_by_state <- major |> count(state, name = "n_major")
  re_by_state <- major |> filter(re6_prefit_eligible) |> count(state, name = "n_re")
  tibble(
...
summarise_major_size_sensitivity <- function(primary_waves) {
  ...
  bind_rows(lapply(seq_len(nrow(grid)), function(i) {
    accepted <- primary_waves |>
      filter(total_cases >= grid$absolute_case_minimum[i],
...
make_v1_comparison <- function(root, primary) {
  ...
  v2_major <- filter(primary, major_epidemic_primary)
  long_v1 <- filter(v1, duration_weeks > 104L)
  split_count <- sum(vapply(seq_len(nrow(long_v1)), function(i) {
    candidates <- filter(v2_major, state == long_v1$state[i], onset_week <= long_v1$end_week[i], end_week >= long_v1$onset_week[i])
    nrow(candidates) >= 2L
  }, logical(1)))
  bind_rows(
...
state_wave_page <- function(df, waves) {
  ...
  minor <- filter(waves, !major_epidemic_primary)
  ...
  p_inc <- ggplot(df, aes(week_start)) +
    ...
    geom_point(data = major |> filter(!wave_end_censored), aes(x = end_week, y = 0), inherit.aes = FALSE, shape = 6, size = 1.8) +
    geom_point(data = major |> filter(!is.na(trough_ratio)), aes(x = preceding_trough_week, y = preceding_trough_incidence), inherit.aes = FALSE, shape = 25, fill = "white", size = 1.8) +
...
write_visual_audit <- function(state_week, primary, path) {
  ...
  for (state_name in sort(unique(state_week$state))) {
    print(state_wave_page(filter(state_week, state == state_name), filter(primary, state == state_name)))
  }
}
...
build_brazil_chik_wave_census_v2 <- function() {
  ...
  primary <- filter(all_waves, segmentation_definition == "primary") |> attach_stability(all_waves)
  ...
  summary <- bind_rows(lapply(c("permissive", "primary", "strict"), function(label) summarise_census(filter(all_waves, segmentation_definition == label), label, uf_lookup$state)))

# Six-panel trajectory figure for v5.0a (q=0.05), same format as
# 28_v4_9.../03_plot_v4_9_six_panel.R. Panel D's subtitle notes the added
# AR(1) dynamic-R residual so the changed R0(t) mechanism is visible.
#
# Usage: Rscript 02_plot_v5_0a_six_panel.R
...
base_dir <- file.path(root, "02_Script/40_renewal_model/29_v5_0_dynamic_R")

plot_v5_0a_six_panel <- function(qval = 0.05) {
  tag <- paste0("q", sprintf("%.2f", qval))
  bundle <- readRDS(file.path(base_dir, "outputs", paste0("v5_0a_", tag), paste0("renewal_ceara_v5_0a_fit_", tag, ".rds")))
  fit <- bundle$fit
  ...
  pars <- c("X", "S_prop", "immune_prop", "R0_t", "R_eff_t", "expected_reported_cases", "C_pred", "delta_R")
  ...
  summaries <- dplyr::bind_rows(
    ...
    summarise_trajectory(draws$delta_R, "delta_R")
  ...
  p_cases <- seed_rect(ggplot()) +
    ...
    labs(title = "Reported cases and posterior prediction",
         subtitle = "Grey band = 2022 seed window (conditioned, excluded from likelihood)",
  ...
  p_infections <- seed_rect(add_interval_layers(ggplot(latent, aes(week_start)), latent, COL_VERMILLION)) +
    ...
    labs(title = "Latent infection incidence", subtitle = "Grey band = 2022 seed window",
  ...
  p_reproduction <- ggplot(reproduction, aes(week_start, colour = number, fill = number)) +
    ...
    x_scale +
    labs(title = "Time-varying reproduction numbers", subtitle = "log_R0(t) = mu_R(t) [harmonic + A_year] + delta_R(t) [AR(1), year-centred]",
         x = NULL, y = "Reproduction number") +
    theme_publication()

  delta_df <- get_summary("delta_R")
  p_delta <- seed_rect(ggplot(delta_df, aes(week_start))) +
    geom_hline(yintercept = 0, linetype = 2, colour = COL_MUTED) +
    geom_ribbon(aes(ymin = q025, ymax = q975), fill = COL_ORANGE, alpha = 0.2) +
    geom_line(aes(y = median), colour = COL_ORANGE, linewidth = 0.7) +
    x_scale +
    labs(title = "Dynamic R0 residual delta_R(t)", subtitle = "Zero-mean within each calendar year by construction",
         x = NULL, y = "delta_R (log-R0 scale)") +
    theme_publication()
  ...
  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_delta | p_attack) +
    patchwork::plot_annotation(
      title = "Chikungunya transmission and population susceptibility in Cear\u00e1, Brazil (2015-2025)",
      subtitle = sprintf("v5.0a dynamic-R (direct serology), q = %.2f (%s) | AR(1) log-R0 residual added to v4.9 backbone",
  ...
  table_dir <- file.path(root, "03_Output/tables/renewal_v5_0_dynamic_R")
  dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(table_dir, recursive = TRUE, showWarnings = FALSE)

  output_base <- paste0("renewal_v5_0a_trajectories_ceara_", tag)
  png_path <- file.path(figure_dir, paste0(output_base, ".png"))
  pdf_path <- file.path(figure_dir, paste0(output_base, ".pdf"))
  table_path <- file.path(table_dir, paste0("renewal_v5_0a_trajectory_summary_", tag, ".csv"))
...
if (sys.nframe() == 0L) plot_v5_0a_six_panel()

# Rio de Janeiro CITY auxiliary -- Model A: global-q CASE-ONLY, frozen
# v4.9. Same Stan file, same priors, same HMC settings family as the STATE
# model (03/04) -- only the geography (city cases + city demography)
# changes. Dispersed prior-valid inits, NOT centred at q=0.010 (per
# instruction Section 6/13 -- must not smuggle in the post-hoc-favoured
# value).
...
make_rj_city_global_q_data <- function(weekly, imports_per_week = 1, year_effect_prior_sd = 0.40) {
...
make_dispersed_init_fn <- function(Y, td14_scalar_inits) {
  # Dispersed but prior-plausible q starts (0.05, 0.10, 0.15, 0.20) --
  # broadly spread around the prior mean (0.10), explicitly NOT centred on
  # 0.010, per instruction.
  q_starts <- c(0.05, 0.10, 0.15, 0.20)
  ...
  function(chain_id = 1L) {
    ...
    list(
      ...
      z_year = rep(offset / 0.40, Y), z_geo = numeric(0),
      logit_q = qlogis(q_starts[cid])
    )
  }
}

run_fit_rj_city_case_only <- function() {
  ...
  prepared <- make_rj_city_global_q_data(weekly)
  Y <- prepared$stan_data$Y
  message("RJ CITY case-only Stan data: N=", prepared$stan_data$N, " Y=", Y, " J_sero=", prepared$stan_data$J_sero)

  scalar_inits <- load_td14_scalar_inits(root)
  init_fn <- make_dispersed_init_fn(Y, scalar_inits)
  message("Initial values (per chain, dispersed 0.05/0.10/0.15/0.20, NOT centred at 0.010):")
  for (i in 1:4) {
    v <- init_fn(i)
  ...
  bundle <- list(fit = fit, stan_data = prepared$stan_data, weekly_data = weekly,
                  config = c(settings, list(model_version = "rio_de_janeiro_CITY_v4_9_global_q_case_only",
                                             elapsed_seconds = elapsed_seconds, seed = 20260915L)),
                  hmc = hmc)
  fit_path <- file.path(out_dir, "rj_city_global_q_case_only.rds")
  saveRDS(bundle, fit_path)
  message(sprintf("[rj city case-only] HMC gate: %s | %.0fs | %s", if (hmc$hmc_pass) "PASS" else "FAIL", elapsed_seconds, fit_path))
  invisible(bundle)
}

if (sys.nframe() == 0L) run_fit_rj_city_case_only()

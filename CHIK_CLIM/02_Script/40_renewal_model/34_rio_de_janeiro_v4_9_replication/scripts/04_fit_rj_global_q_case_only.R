# Rio de Janeiro Phase 1 -- global-q CASE-ONLY frozen v4.9 fit.
# Reuses the EXACT SAME generic Stan file already validated for Bahia/PE
...
# CE/BA/PE. J_sero=0 (case-only, Phase 1 -- no serology). No seed (Section
# 5 input audit found no qualifying RJ inter-wave gap). NO Pernambuco
# ridge/curved reparameterisation imported.
...
LOGIT_Q_PRIOR_MEAN <- qlogis(0.10) # SAME prior as CE/BA/PE -- not tuned for RJ
LOGIT_Q_PRIOR_SD <- 1.0

make_rj_global_q_data <- function(weekly, imports_per_week = 1, year_effect_prior_sd = 0.40) {
  ...
  prepared$stan_data$is_seed <- integer(n) # no seed -- Section 5 found no qualifying RJ gap
  ...
  # Case-only: J_sero=0 (Phase 1, no serology per instruction)
...
make_rj_init_fn <- function(Y, td14_scalar_inits) {
  # Dispersed but prior-valid: q centred on the prior mean with modest
  # offsets (same convention validated for CE/BA/PE's global-q models),
  # NOT the extreme 0.02/0.05/0.10/0.20 spread that captured a pathological
  # mode in the Pernambuco spatial diagnostics.
...
run_fit_rj_global_q_case_only <- function() {
  ...
  message("RJ case-only Stan data: N=", prepared$stan_data$N, " Y=", Y, " J_sero=", prepared$stan_data$J_sero)
  ...
  bundle <- list(fit = fit, stan_data = prepared$stan_data, weekly_data = weekly,
                  config = c(settings, list(model_version = "rio_de_janeiro_v4_9_global_q_case_only",
                                             elapsed_seconds = elapsed_seconds, seed = 20260915L,
                                             logit_q_prior_mean = LOGIT_Q_PRIOR_MEAN, logit_q_prior_sd = LOGIT_Q_PRIOR_SD)),
                  hmc = hmc)
  fit_path <- file.path(out_dir, "rj_global_q_case_only.rds")
  saveRDS(bundle, fit_path)
  message(sprintf("[rj case-only] HMC gate: %s | %.0fs | %s", if (hmc$hmc_pass) "PASS" else "FAIL", elapsed_seconds, fit_path))
  invisible(bundle)
}

if (sys.nframe() == 0L) run_fit_rj_global_q_case_only()

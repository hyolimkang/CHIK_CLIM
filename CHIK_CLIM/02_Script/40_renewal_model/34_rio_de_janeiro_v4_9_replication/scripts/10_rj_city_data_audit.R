...
city_panel_full <- panel |> filter(muni6 == MUNI6) |> arrange(week_start)
...
city_cases <- city_panel_full |> filter(week_start >= DATE_START, week_start <= DATE_END) |>
...
city_demography <- readRDS(demography_path) |> filter(week_start >= DATE_START, week_start <= DATE_END)

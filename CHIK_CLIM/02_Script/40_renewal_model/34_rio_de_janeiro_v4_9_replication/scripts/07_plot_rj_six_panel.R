# Rio de Janeiro -- established-convention 6-panel trajectory figure (SAME
# layout/theme as
...
# Case-only (no serology in Phase 1) -> 6 panels (A-F), no panel G.
...
plot_rj_panel <- function(tag = "caseonly", fit_path = NULL, show_u10 = FALSE) {
  ...
  p_cases <- ggplot() +
    ...
    x_scale +
    labs(title = "Reported cases and posterior prediction", subtitle = "No seed mechanism used (no RJ gap qualifies)",
  ...
  if (show_u10) {
    ...
    # STATEWIDE trajectory -- same convention as PE's U14 panel F. NOT a
    # state=city equivalence claim -- see RJ_U10_CASEONLY_VALIDATION.md.
  ...
  subtitle_model <- sprintf("Frozen v4.9 + global q ESTIMATED (case-only, Phase 1) (%s)",
                             if (bundle$hmc$hmc_pass) "HMC PASS" else "HMC NEAR-PASS/FAIL -- see RJ_GLOBAL_Q_CASE_ONLY_REPORT.md")

  figure <- (p_cases | p_infections) / (p_immunity | p_reproduction) / (p_ascertainment | p_attack) +
    patchwork::plot_annotation(title = "Chikungunya transmission in Rio de Janeiro, Brazil (2015-2025) -- case-only",
  ...
  table_path <- file.path(table_dir, paste0("rio_de_janeiro_trajectory_summary_", tag, ".csv"))

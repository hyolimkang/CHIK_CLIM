# Bahia spatial-turnover diagnostic -- Section 7 (heatmaps) and the
...
metrics <- read_csv(file.path(table_dir, "bahia_wave_turnover_metrics.csv"), show_col_types = FALSE)
...
pairwise <- metrics |> filter(comparison_type == "all_pairwise")
...
library(patchwork)
combined_corr <- p_corr + labs(subtitle = "1.0 = identical rank order; 0 = no association") +
  patchwork::plot_annotation(title = "Bahia wave-by-wave turnover matrix: Section 7")
ggsave(file.path(figure_dir, "bahia_wave_pairwise_correlation_heatmap.png"), p_corr, width = 180, height = 160, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "bahia_wave_pairwise_correlation_heatmap.png"))

overlap_panel <- (p_top10 | p_top20) / p_contrib80
ggsave(file.path(figure_dir, "bahia_top_municipality_overlap_heatmap.png"), overlap_panel, width = 260, height = 260, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "bahia_top_municipality_overlap_heatmap.png"))

# ---- Section 4C/4D: case-contributor turnover (adjacent waves) -----------
adjacent <- metrics |> filter(comparison_type == "adjacent") |>
...
p_turnover <- ggplot(turnover_long, aes(wave_to, share, fill = source)) +
  ...
  labs(title = "Bahia: share of each wave's cases from previously low/zero-incidence municipalities",
...
ggsave(file.path(figure_dir, "bahia_wave_case_contributor_turnover.png"), p_turnover, width = 220, height = 140, units = "mm", dpi = 300, bg = "white")
message("[saved] ", file.path(figure_dir, "bahia_wave_case_contributor_turnover.png"))

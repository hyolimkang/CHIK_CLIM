# Bahia spatial-turnover diagnostic -- Sections 4 & 7.
#
...
contributor_set <- function(df, prop) {
  ...
  df |> filter(lag(cum_share, default = 0) < prop) |> pull(muni6)
}
...
pair_metrics <- function(w1, w2) {
  d1 <- burden |> filter(wave_id == w1)
  d2 <- burden |> filter(wave_id == w2)
  ...
  bottom50_w1 <- d1_ranked |> filter(incidence_rank <= 0.50) |> pull(muni6)
  bottom25_w1 <- d1_ranked |> filter(incidence_rank <= 0.25) |> pull(muni6)
  zero_w1 <- d1 |> filter(reported_cases_iw == 0) |> pull(muni6)
...
write_csv(turnover_metrics, file.path(table_dir, "bahia_wave_turnover_metrics.csv"))
message("\n[saved] ", file.path(table_dir, "bahia_wave_turnover_metrics.csv"))

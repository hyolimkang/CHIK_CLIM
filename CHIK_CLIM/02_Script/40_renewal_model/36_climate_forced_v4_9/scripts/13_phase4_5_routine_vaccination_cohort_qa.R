...
for (t in N_hist:(N_total - 1L)) {
  N_start_full[t + 1L] <- N_start_full[t] + births_full[t] - deaths_full[t] + reconciliation_full[t]
}
is_seed_full <- c(sd_data$is_seed, integer(N_extra))
X_seed_full <- c(sd_data$X_seed, numeric(N_extra))

age_shares_full <- bind_rows(age_shares, bind_rows(lapply(2026:2050, function(y) age_shares |> filter(year == 2025) |> mutate(year = y))))
...
duplicate_check <- cohort_summary |> count(birth_cohort) |> filter(n > 1)
...
p_impact <- ggplot(impact_df |> filter(week_start >= as.Date("2023-01-01")), aes(week_start)) +

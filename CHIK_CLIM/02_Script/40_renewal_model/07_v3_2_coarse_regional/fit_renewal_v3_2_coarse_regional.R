...
suppressPackageStartupMessages({ library(here); library(rstan); library(posterior) })
rstan_options(auto_write = TRUE); options(mc.cores = min(4, parallel::detectCores()))
source(here::here("02_Script/40_renewal_model/07_v3_2_coarse_regional/prepare_renewal_v3_2_coarse_regional_data.R"))
...
hmc_v3_2 <- function(fit) {
  ...
  list(divergences = rstan::get_num_divergent(fit), treedepth_hits = rstan::get_num_max_treedepth(fit),
...
write_v3_2_report <- function(x, compiled, hmc = NULL, elapsed = NULL) {
  ...
  d <- x$diagnostics; q <- x$q_prior
  h <- if (is.null(hmc)) c("- Pilot not run.") else c(
    sprintf("- Elapsed: %.1f seconds; divergences: %d; treedepth hits: %d.", elapsed, hmc$divergences, hmc$treedepth_hits),
    sprintf("- max Rhat: %.3f; Rhat > 1.01: %d; min bulk/tail ESS: %.1f / %.1f; BFMI by chain: %s.",
  ...
  lines <- c("# v3.2 coarse-regional renewal implementation", "", "## Definition", "",
    "- Eight contiguous official-mapping units: Fortaleza, its remainder, Norte/Sobral, Cariri remainder, Juazeiro do Norte, Sertão Central remainder, Quixadá, and Litoral Leste/Jaguaribe.",
    "- Municipality membership and original official macroregion are saved in the assignment CSV. Juazeiro, Quixadá, and Fortaleza are removed from parent regions before aggregation.",
    ...
    "- Weekly regional cases exactly reproduce the Ceará total; births/deaths are non-negative; survey windows map directly to Juazeiro and Quixadá.",
    ...
    "- Municipality deaths retain the explicit v3.0 population-share allocation of Ceará annual deaths before aggregation.",
...
run_v3_2 <- function() {
  ...
  if (run_pilot) {
    d <- x$stan_data
    init <- function() list(alpha_R=log(1.1), sigma_R=.05, z_R=rep(0,d$N-1),
    ...
    timing <- system.time(fit <- rstan::sampling(model, data=d, chains=4, iter=iter, warmup=warmup,
      seed=32052018, init=init, control=list(adapt_delta=.90,max_treedepth=12), refresh=max(1L,floor(iter/10))))
    elapsed <- unname(timing["elapsed"]); h <- hmc_v3_2(fit)
    saveRDS(list(fit=fit, prepared=x, pilot_diagnostics=h, elapsed_seconds=elapsed),
      here::here("02_Script/stan/renewal_ceara_v3_2_coarse_regional_pilot.rds"))
  }
  report <- write_v3_2_report(x, TRUE, h, elapsed); message("[report] ", report)
